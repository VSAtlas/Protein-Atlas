...
bad_csv = out_root / cfg.get(
    "IDENTIFY_UNMAPPED_CSV", "fda_mapping_unmapped_or_no_smiles.csv"
)
# Try to import alias tables from chemdb; fallback to in-code defaults below
try:
    from chemdb.chem_alias_db import (
        SALT_WORDS as _SALT,
        HYDRATE_WORDS as _HYDR,
        FORM_WORDS as _FORM,
        GENERIC_SUFFIX_BONUS as _GENBONUS,
    )
...
# ========= User config =========
SDF_PATH = Path(
    r"E:\PythonProject\protein_automation\extracted_ligands\merged_dedup.sdf"
)
...
CACHE_PATH_RXNORM = Path(
    "rxnorm_cache.json"
)  # retained (used implicitly for rate limiting if extended)
CACHE_PATH_DRUGCENTRAL = Path(
    "drugcentral_cache.json"
)  # legacy (not used with TSV path; kept for compatibility)
...
DRUGCENTRAL_TSV = Path(
    r"E:\PythonProject\protein_automation\ref\drugcentral_structures.tsv"
)
...
def _strip_salt_hydrate_tail(name: str) -> str:
    ...
    tokens = name.replace(",", " ").replace("-", " ").replace("/", " ").lower().split()
...
def load_sdf_rows(sdf_path: Path, one_based: bool) -> pd.DataFrame:
    ...
    for i, mol in enumerate(suppl):
        ...
        if mol is None:
            rows.append(
                {"sdf_index": idx, "sdf_title": None, "smiles": None, "inchikey": None}
            )
            continue
    ...
    log.info(
        f"SDF records parsed: {len(df)} (mol=None: {int(df['sdf_title'].isna().sum())})"
    )
    return df


def _merge_coverage(
    files_df: pd.DataFrame, sdf_df: pd.DataFrame, one_based: bool
) -> float:
    """
    ...
    merged = adj.merge(
        sdf_df[["sdf_index"]], left_on="match_index", right_on="sdf_index", how="left"
    )
...
def infer_indexing(
    files_df: pd.DataFrame, sdf_df: pd.DataFrame, default_one_based: bool
) -> bool:
    """
    ...
    log.info(
        f"Indexing coverage test → zero-based: {cov_zero:.3f}, one-based: {cov_one:.3f}"
    )
...
def sanity_check_index_merge(mapping: pd.DataFrame) -> None:
    ...
    if pct < 80:
        log.warning(
            "Low coverage. Check that your merged_dedup.sdf ordering aligns with prepped ligand numbering."
        )
...
def scan_prepped_files(root: Path) -> pd.DataFrame:
    ...
    if df.empty:
        log.warning(
            "No prepped ligand files found. Check SEARCH_ROOT and filename patterns."
        )
        return df
...
def _parse_pdbqt_element_counts(lines: List[str]) -> Dict[str, int]:
    ...
    for ln in lines:
        ...
        if el is None:
            ...
            el = _ELEMENT_MAP.get(prefix, None) or (
                prefix.capitalize() if prefix else None
            )
        if el:
...
_rx_remark_inchikey = re.compile(
    r"(?i)^REMARK.*?(INCHIKEY|InChIKey)\s*[:=]\s*([A-Z0-9\-]+)"
)
...
def parse_pdbqt_identity(p: Path) -> Dict[str, Any]:
    ...
    out = {
        "path": str(p),
        "remark_smiles": None,
        "remark_inchikey": None,
        "remark_name": None,
        "pdbqt_formula": None,
        "pdbqt_heavy_atoms": None,
    }
    try:
...
def select_representative_pdbqt(files_df: pd.DataFrame) -> pd.DataFrame:
    ...
    def _pick(group: pd.DataFrame) -> pd.Series:
        ...
        g["stage_rank"] = (
            pd.to_numeric(g["stage"], errors="coerce").fillna(-1).astype("int32")
        )
    ...
    reps = (
        files_df.groupby(["scheme", "file_num"], group_keys=False)[cols]
        .apply(_pick)
...
def _calc_formula_heavy(
    smi: Optional[str], smi_neutral: Optional[str]
...
def match_using_pdbqt_features(
    reps_df: pd.DataFrame, sdf_df: pd.DataFrame, default_one_based: bool
) -> pd.DataFrame:
    """
    ...
    by_inchikey = {
        str(ik): i
        for i, ik in zip(sdf_df["sdf_index"], sdf_df["inchikey"].fillna(""))
    ...
    for i, f, h in zip(
        sdf_df["sdf_index"], sdf_df["sdf_formula"], sdf_df["sdf_heavy_atoms"]
    ):
    ...
    for _, row in reps_df.iterrows():
        ...
        matched_rows.append(
            {**row.to_dict(), "sdf_index": target_sdf_index, "match_method": method}
        )
...
def _is_generic_like(s: str) -> bool:
    ...
    if any(c in t for c in [",", ";", "(", ")", "[", "]", "{", "}", "/", "\\"]):
        return False
    if t.count(" ") > 2:
        return False  # allow one or two words
...
def _is_brand_like(s: str) -> bool:
    ...
    if len(parts) > 2:
        return False
    if t == t.lower():
        return False  # brands usually TitleCase / ALLCAPS
    return True
...
def choose_best_generic(
    synonyms: Optional[List[str]], record_title: Optional[str] = None
) -> Optional[str]:
    syns = list(
        dict.fromkeys(
            [_norm_ws(_strip_marks(x)) for x in (synonyms or []) if isinstance(x, str)]
        )
    )
    candidates: List[str] = []
    for s in syns:
    ...
    if not candidates:
        return None
    candidates = sorted(
        set(candidates), key=lambda x: (_score_generic_candidate(x), len(x), x)
    )
    return candidates[0]
...
def fetch_pubchem_view_by_cid(cid: Optional[int]) -> Dict[str, Any]:
    ...
    try:
        ...
        if "UNII" in out["sources"]:
            out["sources"]["UNII"] = [
                re.sub(r"[^A-Z0-9]", "", u.upper())
                for u in out["sources"]["UNII"]
                if isinstance(u, str)
            ]
        return out
    except Exception:
...
def pick_best_display_name(
    candidates: List[str], record_title: Optional[str], iupac: Optional[str]
) -> Optional[str]:
    pool = []
    for c in candidates:
    ...
    if generics:
        generics = sorted(
            set(generics), key=lambda x: (_score_generic_candidate(x), len(x), x)
        )
        return generics[0]
    if iupac:
...
def pubchem_lookup(
    smiles: Optional[str], inchikey_val: Optional[str], cache: dict
) -> Dict[str, Any]:
    ...
    for k in (key_smiles, key_inchi):
        if k:
            ...
            if c:
                if (
                    (not c.get("synonyms"))
                    or (c.get("title") is None)
                    or (c.get("iupac_name") is None)
                ):
                    view = fetch_pubchem_view_by_cid(c.get("cid"))
                    merged_syns = merge_name_buckets(
                        c.get("synonyms", []), view.get("synonyms", []) or []
                    )
                    c["synonyms"] = merged_syns
                    c["title"] = c.get("title") or view.get("record_title")
                    c["iupac_name"] = c.get("iupac_name") or view.get("iupac_name")
                    c["generic"] = choose_best_generic(
                        c.get("synonyms"), c.get("title")
                    )
    ...
    if entry:
        ...
        if entry.get("cid"):
            cache_put(f"CID:{entry['cid']}", entry)
        return entry
...
def enrich_with_pubchem(mapping: pd.DataFrame) -> pd.DataFrame:
    ...
    for k, v in list(cache.items()):
        if (
            ...
            and (
                not v.get("synonyms")
                or v.get("title") is None
                or v.get("iupac_name") is None
            )
        ):
            ...
            if merged_syns:
                v["synonyms"] = merged_syns
            if ttl and not v.get("title"):
                v["title"] = ttl
            if not v.get("title") and view.get("record_title"):
                v["title"] = view.get("record_title")
            if not v.get("iupac_name") and view.get("iupac_name"):
                v["iupac_name"] = view.get("iupac_name")
    ...
    for _, row in mapping.iterrows():
        ...
        if isinstance(s, str) and s not in cache:
            queries.append(("smiles", s, s, ik))
            continue
        if isinstance(sn, str) and sn not in cache:
            queries.append(("smiles", sn, sn, ik))
            continue
    ...
    def get_entry(row):
        ...
        if isinstance(s, str) and s in cache:
            return cache[s]
        if isinstance(sn, str) and sn in cache:
            return cache[sn]
        if isinstance(ik, str) and f"IK:{ik}" in cache:
            return cache[f"IK:{ik}"]
        try:
            cid_int = int(cid_val) if pd.notna(cid_val) else None
        except Exception:
            cid_int = None
        if cid_int and f"CID:{cid_int}" in cache:
            return cache[f"CID:{cid_int}"]
        return {}

    mapping["pubchem_name"] = mapping.apply(lambda r: get_entry(r).get("name"), axis=1)
    mapping["pubchem_cid_resolved"] = mapping.apply(
        lambda r: get_entry(r).get("cid"), axis=1
    )
    mapping["pubchem_record_title"] = mapping.apply(
        lambda r: get_entry(r).get("title"), axis=1
    )
    mapping["generic_name"] = mapping.apply(
        lambda r: get_entry(r).get("generic"), axis=1
    )
    mapping["brand_names"] = mapping.apply(
        lambda r: "; ".join(get_entry(r).get("brands", [])), axis=1
    )
    mapping["pubchem_iupac_name"] = mapping.apply(
        lambda r: get_entry(r).get("iupac_name"), axis=1
    )
    ...
    mapping["pubchem_unii_list"] = mapping.apply(
        lambda r: "; ".join((get_entry(r).get("sources", {}) or {}).get("UNII", []))
        or None,
        axis=1,
    )
    ...
    def pick_display(row):
        return (
            _clean_display_name(row.get("generic_name"))
...
def rxnorm_from_unii(unii: Optional[str]) -> Optional[str]:
    ...
    if not unii:
        return None
    j = rxn_get(
        f"{RXN_BASE}/findRxcuiById.json", {"idtype": "UNII", "id": unii, "allsrc": 1}
    )
    try:
...
def _load_drugcentral_index(tsv_path: Path):
    if not tsv_path.exists():
        log.warning(
            f"DrugCentral TSV not found at {tsv_path}; skipping DrugCentral enrichment."
        )
        return None, None
...
def enrich_with_drugcentral(df: pd.DataFrame) -> pd.DataFrame:
    ...
    for _, row in df.iterrows():
        ...
        if rec is not None:
            ...
            nm = _clean_display_name(
                row.get("generic_name")
                or row.get("pubchem_name")
                or row.get("display_name")
            )
            if nm and _DC_COLS["name"] in _DRUGCENTRAL_DF.columns:
                mm = _DRUGCENTRAL_DF[
                    _DRUGCENTRAL_DF[_DC_COLS["name"]].astype(str).str.lower()
                    == nm.lower()
                ]
                if not mm.empty:
                    rec = mm.iloc[0]
...
def _extract_openfda_names(items: List[dict]) -> Dict[str, Any]:
    ...
    for it in items:
        ...
        if isinstance(of.get("generic_name"), list):
            gen += of["generic_name"]
        if isinstance(of.get("brand_name"), list):
            brand += of["brand_name"]
        if isinstance(of.get("unii"), list):
            unii += of["unii"]
        if isinstance(of.get("rxcui"), list):
            rxcui += of["rxcui"]
...
def main():
    ...
    base_csv = Path("chemdb/data/fda_mapping_from_pdbqt.csv")
