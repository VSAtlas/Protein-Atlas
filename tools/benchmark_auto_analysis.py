...
try:
    from tools.fda_mapping_index import MappingIndex
except Exception:
    from fda_mapping_index import MappingIndex
...
DEFAULT_MAPPING_CSV = os.environ.get("MAPPING_CSV") or str(
    _SCRIPT_ROOT / "chemdb" / "data" / "fda_mapping_from_pdbqt.csv"
)
...
def resolve_rdk_name_quality_from_mapping(rdk_id: str, mapping: MappingIndex) -> str:
    if not mapping:
        return "mapping_missing"
    try:
        return ";".join(mapping.audit_reasons_for_rdk(rdk_id))
    except Exception:
        return "mapping_lookup_failed"
...
def load_controls_and_rdks(
    ...
    for p in pocket_dir.iterdir():
        ...
        if CONTROL_PAT.match(name):
            ...
            quality_reasons = resolve_rdk_name_quality_from_mapping(rdk_id or "", mapping)
            if not rname:
                quality_reasons = ";".join(
                    reason
                    for reason in [quality_reasons, "missing_mapping_name"]
                    if reason
                )
            rdks.append(
                Pose(
                    ...
                    coords=coords,
                    rdk_name=rname,
                    rdk_name_quality_reasons=quality_reasons,
                )
            )
    ...
    for control_id, ctrl in controls:
        ...
        rows.append(
            PairEval(
                ...
                rdk_name_missing=int(not bool(picked.rdk_name)),
                rdk_name_quality_reasons=picked.rdk_name_quality_reasons,
    ...
    with open(path, "w", newline="", encoding="utf-8") as fh:
        ...
        w.writerow(
            [
                ...
                "any_center_within_tol",
                "any_rmsd_within_tol",
                "any_identity_match",
                "num_good_controls",
                "num_confident_matches",
                "num_rdk_missing_names",
                "num_rdk_quality_flagged",
                "passed_4_or_max",
                "near_pass_2to(max-1)",
                "fail_0",
            ]
        ...
        for pdb_id in sorted(by_pdb.keys()):
            ...
            num_missing_names = sum(int(it.rdk_name_missing) for it in items)
            num_quality_flagged = sum(
                1 for it in items if (it.rdk_name_quality_reasons or "").strip()
            )
...
<thead><tr>
  ...
  <th>rdk_name_missing</th><th>rdk_name_quality_reasons</th>
...
"""
        ...
        for r in rows:
            ...
            fh.write(f"<td>{'True' if r.rdk_name_missing else 'False'}</td>")
            fh.write(f"<td>{_html_escape(r.rdk_name_quality_reasons)}</td>")
...
def write_details_xlsx(
    ...
    try:
        ...
        ws.set_column(9, 14, 12)
        ws.set_column(15, 20, 11)
        ws.set_column(21, 26, 32)  # cover all 6 image columns
        # Data rows
        row = 1
        for r in rows:
            ws.write_row(
            ...
            for offset, p in enumerate([r.png_side, r.png_front, r.png_top]):
                if p and Path(p).is_file():
                    try:
                        ws.insert_image(
                            row, 21 + offset, str(p), {"x_scale": 0.35, "y_scale": 0.35}
                        )
                    except Exception:
                        pass
            ...
            for offset, p in enumerate(
                ...
                if p and Path(p).is_file():
                    try:
                        ws.insert_image(
                            row, 24 + offset, str(p), {"x_scale": 0.35, "y_scale": 0.35}
                        )
                    except Exception:
                        pass
            row += 1
    ...
    try:
        ...
        for r in rows:
            ...
            row_idx = ws.max_row
            # overlay
            for i, p in enumerate([r.png_side, r.png_front, r.png_top], start=22):
                try:
                    if p and Path(p).is_file():
                        img = XLImage(str(p))
            ...
            for i, p in enumerate(
                [r.png_pair_side, r.png_pair_front, r.png_pair_top], start=25
            ):
                try:
                    if p and Path(p).is_file():
                        img = XLImage(str(p))
...
def run_analysis(
    ...
    for pdb_dir in pdb_dirs:
        ...
        for variant_value in expand_variants(variant_mode):
            ...
            missing_name_rows = sum(r.rdk_name_missing for r in all_rows)
            quality_flagged_rows = sum(
                1 for r in all_rows if (r.rdk_name_quality_reasons or "").strip()
            )
            ...
            print(
                "[analysis] FDA mapping-name audit: "
                f"missing={missing_name_rows} quality_flagged={quality_flagged_rows}"
            )
            print(
