...
def main():
    ...
    if len(pos) < 3:
        print(f"usage: cap_driver.py <receptor> <ligand> <outdir-or-prefix> "
    ...
    parser.add_argument("--label-top-n-res", dest="label_top_n_res", type=int,
                        default=int(os.environ.get("PYMOL_LABEL_TOP_N_RES", "5")))
    parser.add_argument("--label-cutoff-A", dest="label_cutoff", type=float,
                        default=float(os.environ.get("PYMOL_LABEL_CUTOFF_A", "5.0")))
    parser.add_argument("--viewport", dest="viewport", type=str,
                        default=os.environ.get("PYMOL_VIEWPORT", "1600x1200"))
    parser.add_argument("--transparent", dest="transparent", type=int,
                        default=1 if _get_bool("PYMOL_TRANSPARENT", True) else 0)
    args, unknown = parser.parse_known_args(pos[-len(pos):])  # parse all remaining tokens
    ...
    out_parent = out_path if out_path.suffix == "" and (str(out_path).endswith("/") or out_path.is_dir()) else out_path.parent
