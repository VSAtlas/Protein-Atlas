...
if (!("z_selected" %in% names(df))) {
  stop("Missing required column: z_selected")
}
...
df$z_selected <- suppressWarnings(as.numeric(df$z_selected))
df <- df[!is.na(df$z_selected), ]
if (nrow(df) == 0) {
  stop("No usable z_selected values found in input CSV")
}
...
# Aggregate duplicate target/ligand pairs with max z_selected.
agg <- aggregate(
  z_selected ~ target_id + ligand_name,
  data = df,
  FUN = function(x) max(x, na.rm = TRUE)
)

# Select top-k ligands by global max z_selected (deterministic tie-breaker by name).
ligand_max <- aggregate(
  z_selected ~ ligand_name,
  data = agg,
  FUN = function(x) max(x, na.rm = TRUE)
)
ligand_max <- ligand_max[order(-ligand_max$z_selected, ligand_max$ligand_name), ]
...
plot_title <- if (use_dendrogram) {
  "z_selected heatmap (rows=ligands, cols=targets; dendrograms show similarity)"
} else {
  "z_selected heatmap (rows=ligands, cols=targets)"
}
