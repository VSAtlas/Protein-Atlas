...
def load_run_manifest(
    ...
    """
    relocated_root = os.environ.get("MANIFESTS_DIR")
    canonical = output_root(repo_root, "manifests") / run_id / CANONICAL_MANIFEST_NAME
    candidates = [
        Path(relocated_root).expanduser() / run_id / CANONICAL_MANIFEST_NAME
        if relocated_root
        else None,
        canonical,
        repo_root / "manifests" / run_id / CANONICAL_MANIFEST_NAME,
        output_root(repo_root, "post_docked") / run_id / CANONICAL_MANIFEST_NAME,
        output_root(repo_root, "docked") / run_id / CANONICAL_MANIFEST_NAME,
...
def extract_pocket(
    ...
    ph_norm = ph_label or ""
    composite_key = build_target_id(pdb_norm, variant_norm, ph_norm)
    raw_composite = build_target_id(pdb_id, variant, ph_label)
    ...
    direct_entry = proteins.get(composite_key) or proteins.get(raw_composite)
