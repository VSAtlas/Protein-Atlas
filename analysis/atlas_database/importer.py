...
def _run_paths(repo_root: Path, entry: Mapping[str, Any]) -> dict[str, Path]:
    ...
    return {
        ...
        else output_root(repo_root, "docked") / run_id / "_artifact_archives" / "archive_index.json",
    }
...
def audit_release_inputs(manifest_path: Path, repo_root: Path) -> dict[str, Any]:
    ...
    for entry in iter_run_entries(manifest):
        ...
        completion_count = (
            sum(1 for _ in paths["docked"].rglob("completion_*.json"))
            if paths["docked"].is_dir()
            else 0
        )
        runs.append(
            {
                ...
                "completion_manifest_count": completion_count,
...
def _context_values(key: str, entry: Mapping[str, Any]) -> tuple[str, str, str]:
    ...
    variant = _text(entry.get("variant") or (parts[1] if len(parts) > 1 else "")).upper()
    ph = _normalize_ph(
        entry.get("ph")
        or entry.get("ph_label")
...
def _pocket(entry: Mapping[str, Any]) -> tuple[str, Any, Any]:
    ...
    return _text(details.get("method") or details.get("pocket_method")), details.get("center"), details.get("box_size")
...
def _upsert_context(
    ...
    **values: Any,
    ...
    connection.execute(
        ...
        """,
        (
            run_id, pdb_id, variant, ph, values.get("library"), values.get("status"),
            values.get("failure_reason"), values.get("input_receptor_path"),
            values.get("prepared_receptor_path"), values.get("pocket_method"),
            _json(values["center"]) if values.get("center") is not None else None,
            _json(values["box"]) if values.get("box") is not None else None,
            _json(values["manifest_entry"]) if values.get("manifest_entry") is not None else None,
        ),
    )
...
def _resolve_context(
    ...
    candidates = [
        (str(row[0]), str(row[1]))
        for row in rows
        if (not variant or str(row[0]) == variant)
        and (not ph or str(row[1]) == ph)
    ]
    return candidates[0] if len(candidates) == 1 else (variant, ph)
...
def _ligand(connection: sqlite3.Connection, canonical_id: str, **values: Any) -> int:
    ...
    return int(connection.execute("SELECT ligand_id FROM ligands WHERE canonical_id=?", (canonical_id,)).fetchone()[0])
...
def _pair(connection: sqlite3.Connection, context_id: int, ligand_id: int) -> int:
    ...
    return int(connection.execute("SELECT pair_cell_id FROM pair_cells WHERE receptor_context_id=? AND ligand_id=?", (context_id, ligand_id)).fetchone()[0])


def _manifest_contexts(connection: sqlite3.Connection, run_id: str, manifest: Mapping[str, Any]) -> None:
    ...
    for key, raw in proteins.items():
        ...
        _upsert_context(
            connection, run_id, pdb_id, variant, ph,
            ...
            prepared_receptor_path=_text(details.get("receptor_pdbqt") or details.get("cleaned_pdb")) or None,
            pocket_method=pocket_method or None, center=center, box=box, manifest_entry=raw,
        )


def _completion_files(connection: sqlite3.Connection, run_id: str, docked: Path) -> None:
    if not docked.is_dir():
        return
    for path in sorted(docked.rglob("completion_*.json")):
        try:
        ...
        missing = {canonical_ligand_id(item) for item in payload.get("missing_ligands_after", [])}
        ...
        for ligand_path in payload.get("expected_ligands", []):
            ...
            reason = _text(markers.get(canonical) or markers.get(Path(_text(ligand_path)).name))
            ...
            connection.execute(
                ...
                (pair_cell_id, engine, stage, chunk_id, status, failure_code,
                 failure_reason, completion_path, completion_json)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
                ON CONFLICT(pair_cell_id, engine, stage, chunk_id) DO UPDATE SET
                ...
                (pair_id, engine, stage, chunk, status,
                 "missing_engine_output" if is_missing else ("engine_failure" if reason else None),
                 reason or ("expected ligand has no engine output" if is_missing else None),
                 str(path), _json(payload)),
            )
    ...
    with path.open("r", encoding="utf-8", newline="") as handle:
        for row in csv.DictReader(handle):
            ...
            context_id = _upsert_context(
                ...
                center=[_float(row.get(name)) for name in ("center_x", "center_y", "center_z")],
                box=[_float(row.get(name)) for name in ("box_x", "box_y", "box_z")],
            )
            canonical = canonical_ligand_id(row.get("ligand_base") or row.get("ligand_file") or row.get("ligand"))
            if not canonical:
                continue
            ligand_id = _ligand(connection, canonical, display_name=_text(row.get("ligand_display")) or None)
            pair_id = _pair(connection, context_id, ligand_id)
            valid_token = _text(row.get("pose_valid_any"))
            pose_valid = 1 if valid_token == "1" else (0 if valid_token == "0" else None)
            score = _float(row.get("z_selected"))
            has_numeric = score is not None or any(_float(row.get(name)) is not None for name in ("selected_docking_score", "consensus_score", "final_score"))
            if pose_valid == 0:
                status = "invalid"
            elif pose_valid == 1:
                status = "valid"
                ...
                status = "unsuccessful"
            reason = _text(row.get("pose_invalid_reason_top") or row.get("stage_fallback_reason") or row.get("dud_eval_status_reason"))
            connection.execute(
                ...
                final_score=?, final_rank=?, source_csv=?, result_json=? WHERE pair_cell_id=?""",
                (status, "pose_invalid" if pose_valid == 0 else ("no_numeric_result" if not has_numeric else None),
                 reason or None, pose_valid, int(_text(row.get("is_control")) == "1"),
                 int(_text(row.get("is_decoy")) == "1"), score,
                 _text(row.get("z_selected_source")) or None,
                 _float(row.get("selected_docking_score")), _float(row.get("consensus_score")),
                 _float(row.get("final_score")), _int(row.get("final_rank")),
                 _text(row.get("source_csv")) or None, _json(row), pair_id),
            )
    ...
    for group in index.get("groups", []):
        ...
        for entry in group.get("entries", []):
            if not isinstance(entry, Mapping):
                continue
            candidate = canonical_ligand_id(entry.get("member_name") or entry.get("original_path"))
            ligand_row = connection.execute("SELECT ligand_id FROM ligands WHERE canonical_id=?", (candidate,)).fetchone()
            pair_id = _pair(connection, context_id, int(ligand_row[0])) if ligand_row else None
            connection.execute(
                ...
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)""",
                (run_id, context_id, pair_id, _text(entry.get("stage_dir")) or None,
                 _text(entry.get("mode")) or None, _text(entry.get("original_path")) or None,
                 _text(entry.get("archive_path")) or None, _text(entry.get("member_name")) or None,
                 _text(entry.get("sha256")) or None, _int(entry.get("size_bytes")),
                 _text(entry.get("file_type")) or None, int(bool(group.get("verified"))), _json(entry)),
            )


def _summary(connection: sqlite3.Connection, audit: Mapping[str, Any], database_path: Path) -> dict[str, Any]:
    counts = {
        table: int(connection.execute(f"SELECT COUNT(*) FROM {table}").fetchone()[0])
        for table in ("runs", "receptor_contexts", "ligands", "pair_cells", "docking_attempts", "artifacts")
    }
    statuses = {
        row[0]: int(row[1])
        for row in connection.execute("SELECT final_status, COUNT(*) FROM pair_cells GROUP BY final_status ORDER BY final_status")
    }
    return {
        ...
        "failure_complete": audit["failure_complete_run_count"] == audit["run_count"]
    ...
    *,
    ...
    try:
        ...
        connection.execute(
            ...
            (_text(manifest["release_id"]), _text(manifest["schema_version"]),
             _text(manifest.get("title")) or None, _text(manifest.get("created_at")) or None,
             _sha256(manifest_path), _json(manifest), now),
        )
        for name, value in manifest["scientific_policies"].items():
            connection.execute("INSERT INTO scientific_policies VALUES (?, ?, ?)", (manifest["release_id"], name, _json(value)))
        for entry in iter_run_entries(manifest):
            ...
            connection.execute(
                ...
                (run_id, manifest["release_id"], _text(run_manifest.get("status")) or None,
                 str(paths["manifest"]), _sha256(paths["manifest"]),
                 _json(run_manifest.get("command")), _json(run_manifest.get("git")),
                 _json(run_manifest.get("paths")), _json(run_manifest.get("timing")),
                 _json(run_manifest.get("resources")), _json(run_manifest)),
            )
