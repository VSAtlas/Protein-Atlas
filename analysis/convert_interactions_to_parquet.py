...
def _resolve_input_path(repo_root: Path, run_id: str, raw_input: Optional[str]) -> Path:
    ...
    raise FileNotFoundError(
        "No default input CSV found; expected data/<run_id>/master_rows.csv "
        "or data/<run_id>/heatmap_input.csv"
    )
...
def _resolve_output_dir(repo_root: Path, run_id: str, raw_out_dir: Optional[str]) -> Path:
    if raw_out_dir:
        ...
        return out_dir
    return repo_root / "data" / run_id / "dataset" / "interactions"


def _validate_input_columns(columns: Sequence[str]) -> None:
    colset = set(columns)
    if "t_selected" not in colset:
...
def _normalize_chunk(
    ...
    t_selected = _coerce_float_series(chunk["t_selected"], "float64")
    valid_t = t_selected.notna()
    if not bool(valid_t.any()):
        return out

    chunk = chunk.loc[valid_t].copy()
    out = out.loc[valid_t].copy()
    t_selected = t_selected.loc[valid_t]
    out["t_selected"] = t_selected
