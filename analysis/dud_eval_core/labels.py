...
from analysis.dud_eval_core.schema import guess_ligfile_col, guess_score_col
...
def compute_decoy_stats_from_long_csv(
    ...
    *,
    ...
    Decoy/active labels are derived from the ligand filename using
    parse_name_and_label, and best scores are defined as the minimum score per
    ligand. If assume_unlabeled_decoys is true and the file contains no
    active/decoy filename labels at all, all ligands are treated as decoys; this
    supports arbitrary libraries selected as the run's DUD background.
