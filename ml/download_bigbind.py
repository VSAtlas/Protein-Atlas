...
def _safe_extract(tar: tarfile.TarFile, destination: Path) -> None:
    ...
    for member in tar.getmembers():
        ...
        if not str(member_target).startswith(str(destination)):
...
def extract_bigbind_tar(*, tar_path: Path, destination_dir: Path, force: bool) -> Path:
    extract_root = destination_dir / "BigBindV1.5"
    if extract_root.exists() and not force:
        print(f"[ml] Reusing extracted directory: {extract_root}")
        return extract_root
...
def build_arg_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Download and extract BigBind v1.5 data archive."
    )
    parser.add_argument(
        "--url",
        required=True,
        help="URL for BigBindV1.5.tar from the official BigBind release page.",
    )
    parser.add_argument(
        "--out-dir",
        default="ml/data/bigbind",
