...
def _parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "post_docked, processed_pdbs, and configs are relocated and created."
        ),
    )
    parser.add_argument(
        "--prepped-archive",
...
def _extract_prepped_archive(
    ...
    with tempfile.TemporaryDirectory(prefix="prepped_extract_") as tmp:
        tmp_root = Path(tmp)
        cmd = [
            "tar",
...
def main() -> int:
    ...
    output_root = (
        (all_dirs_root / "processed_pdbs").resolve() if all_dirs_root else None
    )
    configs_root = (all_dirs_root / "configs").resolve() if all_dirs_root else None
