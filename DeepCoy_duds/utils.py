...
def _load_tensorflow():
    try:
        ...
        try:
            ...
            raise RuntimeError(
                "TensorFlow is required for DeepCoy utilities. Install with: "
...
def to_graph(smiles, dataset):
    ...
    for atom in mol.GetAtoms():
        if dataset == "qm9" or dataset == "cep":
            ...
            dataset == "zinc" or dataset == "zinc_phosphorus"
    ...
    for step in transition_freqs:
        ...
        for node, freq in step:
            if freq > 0:
                ...
                s[node] = math.log10(max(min(1 / freq, 10000000) / 1000.0, 10.0))
            else:
                s[node] = 10.0
...
def transition_freqs_edge_to_dense(
    transition_freqs, maximum_vertice_num, num_edge_types=3
):
    new_freqs_sequence = []
    for step in transition_freqs:
        new_freq_seq = []
