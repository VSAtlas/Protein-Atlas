...
from DeepCoy_duds.utils import dataset_info, ThreadedIterator
...
def _load_tensorflow():
    try:
        ...
        if not hasattr(tf_mod, "contrib"):
            tf_mod.contrib = types.SimpleNamespace(rnn=tf_mod.nn.rnn_cell)
        return tf_mod
    except Exception:
        try:
...
class ChemModel(object):
    ...
    def __init__(self, args):
        ...
        self.log_dir = args.get("--log_dir") or "."
        os.makedirs(self.log_dir, exist_ok=True)
        self.log_file = os.path.join(
            self.log_dir, "%s_log_%s.json" % (self.run_id, dataset)
        )
        self.best_model_file = os.path.join(
            self.log_dir, "%s_model.pickle" % self.run_id
        )

        with open(
            os.path.join(self.log_dir, "%s_params_%s.json" % (self.run_id, dataset)),
            "w",
        ) as f:
            json.dump(params, f)
        print(
        ...
        if params["subgraph_freq_file"]:
            ...
            print(
                "Loaded subgraph frequency dictionary from %s"
    ...
    def load_data(self, file_name, is_training_data: bool):
        def resolve_cache_path(cache_path):
            ...
            if os.path.isabs(cache_path):
                return cache_path
            if self.data_dir:
                return os.path.join(self.data_dir, cache_path)
            return cache_path

        def select_cache_path():
            if is_training_data:
                return self.args.get("--train-cache") or self.params.get("train_cache")
            return self.args.get("--valid-cache") or self.params.get("valid_cache")

        cache_path = resolve_cache_path(select_cache_path())
        if cache_path and os.path.isfile(cache_path):
            try:
                print("Loading preprocessed cache from %s" % cache_path)
                with open(cache_path, "rb") as f:
                    cached = pickle.load(f)
                processed = cached.get("processed")
                if processed is None:
                    raise ValueError("Cache missing 'processed' payload.")
                meta = cached.get("meta", {})
                self.max_num_vertices = max(
                    self.max_num_vertices, int(meta.get("max_num_vertices", 0))
                )
                self.num_edge_types = max(
                    self.num_edge_types, int(meta.get("num_edge_types", 0))
                )
                self.annotation_size = max(
                    self.annotation_size, int(meta.get("annotation_size", 0))
                )
                if cached.get("dataset") and cached.get("dataset") != self.params.get(
                    "dataset"
                ):
                    print("Warning: cache dataset does not match current dataset.")
                return processed
            except Exception as e:
                print(
                    "Warning: failed to load cache %s (%s). Falling back to raw JSON."
                    % (cache_path, e)
                )
        ...
        if not data:
            raise ValueError("No data loaded from %s" % full_path)

        # Get some common data out:
        num_fwd_edge_types = len(utils.bond_dict) - 1
        local_max_num_vertices = 0
        for g in data:
            max_in = max([v for e in g["graph_in"] for v in [e[0], e[2]]], default=0)
            max_out = max([v for e in g["graph_out"] for v in [e[0], e[2]]], default=0)
            local_max_num_vertices = max(local_max_num_vertices, max_in, max_out)

        local_num_edge_types = num_fwd_edge_types * (
            1 if self.params["tie_fwd_bkwd"] else 2
        )
        local_annotation_size = len(data[0]["node_features_in"][0])

        self.max_num_vertices = max(self.max_num_vertices, local_max_num_vertices)
        self.num_edge_types = max(self.num_edge_types, local_num_edge_types)
        self.annotation_size = max(self.annotation_size, local_annotation_size)

        processed = self.process_raw_graphs(data, is_training_data, file_name)

        write_cache = bool(self.args.get("--write-cache"))
        if cache_path and write_cache:
            ...
            payload = {
                ...
                "is_training_data": bool(is_training_data),
                "processed": processed,
                "meta": {
                    "max_num_vertices": local_max_num_vertices,
                    "num_edge_types": local_num_edge_types,
                    "annotation_size": local_annotation_size,
                },
            }
            with open(cache_path, "wb") as f:
                pickle.dump(payload, f, pickle.HIGHEST_PROTOCOL)
            print("Wrote preprocessed cache to %s" % cache_path)
    ...
    def make_model(self):
        ...
        with tf.variable_scope("graph_model"):
            ...
            initial_state_in = self.get_node_embedding_state(
                self.placeholders["initial_node_representation_in"], source=True
            )
            initial_state_out = self.get_node_embedding_state(
                self.placeholders["initial_node_representation_out"], source=False
            )
            ...
            if self.params["use_graph"]:
                if self.params["residual_connection_on"]:
                    self.ops[
                        "final_node_representations_in"
                    ] = self.compute_final_node_representations_with_residual(
                        initial_state_in,
                        tf.transpose(
                            self.placeholders["adjacency_matrix_in"], [1, 0, 2, 3]
                        ),
                        "_encoder",
                    )
                    self.ops[
                        "final_node_representations_out"
                    ] = self.compute_final_node_representations_with_residual(
                        initial_state_out,
                        tf.transpose(
                            self.placeholders["adjacency_matrix_out"], [1, 0, 2, 3]
                        ),
                        "_encoder",
                    )
