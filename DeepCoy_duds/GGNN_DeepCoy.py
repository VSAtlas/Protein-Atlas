    ...
    def load_data(self, file_name, is_training_data: bool):
        ...
        if cache_path and os.path.isfile(cache_path):
            try:
                ...
                self.max_num_vertices = max(self.max_num_vertices, int(meta.get("max_num_vertices", 0)))
                self.num_edge_types = max(self.num_edge_types, int(meta.get("num_edge_types", 0)))
                self.annotation_size = max(self.annotation_size, int(meta.get("annotation_size", 0)))
                if cached.get("dataset") and cached.get("dataset") != self.params.get("dataset"):
                    print("Warning: cache dataset does not match current dataset.")
                return processed
            except Exception as e:
                print("Warning: failed to load cache %s (%s). Falling back to raw JSON." % (cache_path, e))
        ...
        local_num_edge_types = num_fwd_edge_types * (1 if self.params['tie_fwd_bkwd'] else 2)
        ...
        with self.graph.as_default():
            for epoch in range(1, self.params['num_epochs'] + 1):
                if not self.params['generation']:
                    ...
                    self.save_model(os.path.join(self.log_dir, self.run_id + "_" + str(epoch)+("_%s.pickle" % (self.params["dataset"]))))
