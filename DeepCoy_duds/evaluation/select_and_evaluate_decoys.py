...
def select_and_evaluate_decoys(
    ...
    seen = set()
    data = [
        d for d in data if Chem.MolFromSmiles(d[0]).GetNumHeavyAtoms() > min_active_size
    ]
    ...
    for key in results_dict:
        ...
        while True:
            count_success = sum(
                [
                    i[4] < prop_max_diff
                    and i[6] < max_basic_diff
                    and i[7] < max_sa_diff
                    and i[8] < max_dg_score
                    and i[9] < max_lads_score
            ...
            if count_success < num_cand_dec_per_act:
                ...
                sorted_mols_success.append(
                    [
                        ...
                        for i in sorted(
                            results_dict[key][0:max_idx_cmpd],
    ...
    for act_res in sorted_mols_success:
        ...
        for ent in act_res:
            ...
            if (
                AllChem.EmbedMolecule(m, randomSeed=42) != -1
                and ent[1] not in decoy_mols_gen
    ...
    if dataset == "dude_ext":
        ...
        actives_feat = decoy_utils.calc_dataset_props_dekois(
            active_mols_gen, verbose=True
        )
        decoys_feat = decoy_utils.calc_dataset_props_dekois(
            decoy_mols_gen, verbose=True
        )
    elif dataset == "MUV":
    ...
    try:
        dec_results.extend(
            list(decoy_utils.calc_xval_performance(actives_feat, decoys_feat, n_jobs=1))
        )
    except:
