...
def test_deterministic_merge_parallel_vs_sequential(monkeypatch, tmp_path):
    ...
    def stub_chembl(*args, **kwargs):
        return ["A", "B"], {"counts": {"targets": 1, "assays": 1, "activities": 2}, "cached": False}

    def stub_bindingdb(*args, **kwargs):
        return ["B", "C"], {"counts": {"pdb_hits": 1, "uniprot_hits": 1}, "cached": False}

    def stub_iuphar(*args, **kwargs):
        return ["C", "D"], {"counts": {"targets": 1, "interactions": 2}, "cached": False}
    ...
    seq = query_external_sources("U", [], "PDB", sources, cache_dir, opts, max_workers=1)
    par = query_external_sources("U", [], "PDB", sources, cache_dir, opts, max_workers=4)
    assert seq == par
...
def test_cached_source_merge_order(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr("generate_dud_library.fetch_chembl_smiles", stub_chembl)
    monkeypatch.setattr("generate_dud_library.fetch_bindingdb_smiles", stub_bindingdb)
    monkeypatch.setattr("generate_dud_library.fetch_iuphar_smiles", lambda *a, **k: ([], {"counts": {}, "cached": False}))

    sources = ["chembl", "bindingdb", "iuphar"]
    opts = make_opts()

    seq = query_external_sources("U", [], "PDB", sources, cache_dir, opts, max_workers=1)
    par = query_external_sources("U", [], "PDB", sources, cache_dir, opts, max_workers=2)
    assert seq == par
