...
Options:
    ...
    --train-cache FILE       Preprocessed cache path for training data
    --valid-cache FILE       Preprocessed cache path for validation data
    --write-cache            Write cache files if cache paths are provided
"""
...
class DenseGGNNChemModel(ChemModel):
    ...
    def prepare_specific_graph_model(self) -> None:
        ...
        expanded_h_dim = (
            self.params["hidden_size"] + self.params["hidden_size"] + 1
        )  # 1 for focus bit
        ...
        self.placeholders["initial_node_representation_in"] = tf.placeholder(
            tf.float32,
            [None, None, self.params["hidden_size"]],
            name="node_features_in",
        )  # padded node symbols
        self.placeholders["initial_node_representation_out"] = tf.placeholder(
            tf.float32,
            [None, None, self.params["hidden_size"]],
            name="node_features_out",
        )  # padded node symbols
        # mask out invalid node
        self.placeholders["node_mask_in"] = tf.placeholder(
            tf.float32, [None, None], name="node_mask_in"
        )  # [b x v]
        self.placeholders["node_mask_out"] = tf.placeholder(
            tf.float32, [None, None], name="node_mask_out"
        )  # [b x v]
        self.placeholders["num_vertices"] = tf.placeholder(tf.int32, ())
        # adj for encoder
        self.placeholders["adjacency_matrix_in"] = tf.placeholder(
            tf.float32,
            [None, self.num_edge_types, None, None],
            name="adjacency_matrix_in",
        )  # [b, e, v, v]
        self.placeholders["adjacency_matrix_out"] = tf.placeholder(
            tf.float32,
            [None, self.num_edge_types, None, None],
            name="adjacency_matrix_out",
        )  # [b, e, v, v]

        # labels for node symbol prediction
        self.placeholders["node_symbols_in"] = tf.placeholder(
            tf.float32, [None, None, self.params["num_symbols"]]
        )  # [b, v, edge_type]
        self.placeholders["node_symbols_out"] = tf.placeholder(
            tf.float32, [None, None, self.params["num_symbols"]]
        )  # [b, v, edge_type]
        # node symbols used to enhance latent representations
        self.placeholders["latent_node_symbols_in"] = tf.placeholder(
            tf.float32,
            [None, None, self.params["hidden_size"]],
            name="latent_node_symbol_in",
        )  # [b, v, h]
        self.placeholders["latent_node_symbols_out"] = tf.placeholder(
            tf.float32,
            [None, None, self.params["hidden_size"]],
            name="latent_node_symbol_out",
        )  # [b, v, h]

        # mask out cross entropies in decoder
        self.placeholders["iteration_mask_out"] = tf.placeholder(
            tf.float32, [None, None]
        )  # [b, es]
        # adj matrices used in decoder
        self.placeholders["incre_adj_mat_out"] = tf.placeholder(
            tf.float32,
            [None, None, self.num_edge_types, None, None],
            name="incre_adj_mat_out",
        )  # [b, es, e, v, v]
        # distance
        self.placeholders["distance_to_others_out"] = tf.placeholder(
            tf.int32, [None, None, None], name="distance_to_others_out"
        )  # [b, es,v]
        ...
        self.placeholders["node_sequence_out"] = tf.placeholder(
            tf.float32, [None, None, None], name="node_sequence_out"
        )  # [b, es, v]
        # mask out invalid edge types at each iteration step
        self.placeholders["edge_type_masks_out"] = tf.placeholder(
            tf.float32,
            [None, None, self.num_edge_types, None],
            name="edge_type_masks_out",
        )  # [b, es, e, v]
        # ground truth edge type labels at each iteration step
        self.placeholders["edge_type_labels_out"] = tf.placeholder(
            tf.float32,
            [None, None, self.num_edge_types, None],
            name="edge_type_labels_out",
        )  # [b, es, e, v]
        # mask out invalid edge at each iteration step
        self.placeholders["edge_masks_out"] = tf.placeholder(
            tf.float32, [None, None, None], name="edge_masks_out"
        )  # [b, es, v]
        # ground truth edge labels at each iteration step
        self.placeholders["edge_labels_out"] = tf.placeholder(
            tf.float32, [None, None, None], name="edge_labels_out"
        )  # [b, es, v]
        # ground truth labels for whether it stops at each iteration step
        self.placeholders["local_stop_out"] = tf.placeholder(
            tf.float32, [None, None], name="local_stop_out"
        )  # [b, es]
        # z_prior sampled from standard normal distribution
        self.placeholders["z_prior"] = tf.placeholder(
            tf.float32, [None, None, self.params["encoding_size"]], name="z_prior"
        )  # prior z ~ normal distribution - output molecule
        self.placeholders["z_prior_in"] = tf.placeholder(
            tf.float32, [None, None, self.params["hidden_size"]], name="z_prior_in"
        )  # prior z ~ normal distribution - input molecule
        ...
        self.placeholders["overlapped_edge_features_out"] = tf.placeholder(
            tf.int32, [None, None, None], name="overlapped_edge_features_out"
        )  # [b, es, v]
        # weights for loss (based on compound freqs)
        self.placeholders["new_compound_freqs_out"] = tf.placeholder(
            tf.float32, [None, None, None], name="new_compound_freqs_out"
        )  # [b, es, v]
        self.placeholders["new_compound_freqs_edge_out"] = tf.placeholder(
            tf.float32, [None, None, None, None], name="new_compound_freqs_out"
        )  # [b, es, e, v]
        ...
        for direc in ["in", "out"]:
            for i in range(self.num_edge_types):
                ...
                self.weights["edge_type_output_%d_%s" % (i, direc)] = tf.Variable(
    ...
    def compute_mean_and_logvariance(self):
        ...
        avg_last_h_out = tf.reduce_sum(
            ...
            * self.ops["graph_state_mask_out"],
            ...
            self.ops["graph_state_mask_out"], 1
        )  # [b, 1, h] - mask out unused nodes
        mean_out = (
            tf.matmul(avg_last_h_out, self.weights["mean_weights_out"])
            + self.weights["mean_biases_out"]
        )
        logvariance_out = (
        ...
        logvariance_out_ex = tf.reshape(
            tf.tile(tf.expand_dims(logvariance_out, 1), [1, v, 1]), [-1, out_dim]
        )

        # PER VERTEX ENCODING - input molecule
        reshped_last_h = tf.reshape(
            self.ops["final_node_representations_in"], [-1, h_dim]
        )
        mean = (
    ...
    def sample_with_mean_and_logvariance(self):
        ...
        z_sampled = tf.cond(
            ...
            lambda: tf.add(
                self.ops["mean_out"],
                tf.multiply(tf.sqrt(tf.exp(self.ops["logvariance_out"])), z_prior),
            ),
        )  # non-standard normal
        ...
        roll_shift = tf.cast(
            tf.reduce_min(tf.reduce_sum(self.ops["graph_state_mask_in"], 1)), tf.int32
        )  # Shift by smallest number of atoms in smallest mol
        inverted_mask = (
            tf.ones(tf.shape(self.ops["graph_state_mask_in"]))
            - self.ops["graph_state_mask_in"]
        )
        update_vals = (
            self.placeholders["z_prior_in"] * inverted_mask
        )  # Extra nodes from N(0,1)
        mean = tf.reshape(
            tf.add(mean, update_vals), [-1, h_dim]
        )  # Fill in extra vertices with random noise from N(0,1)
        ...
        atten_masks_c = (
            tf.tile(tf.expand_dims(self.ops["graph_state_mask_out"], 2), [1, 1, v, 1])
            * LARGE_NUMBER
            - LARGE_NUMBER
        )
        atten_masks_yi = (
            tf.tile(tf.expand_dims(self.ops["graph_state_mask_out"], 1), [1, v, 1, 1])
            * LARGE_NUMBER
            - LARGE_NUMBER
        )
        ...
        atten_c = tf.tile(
            tf.expand_dims(tf.reshape(mean, [-1, v, h_dim]), 2), [1, 1, v, 1]
        )  # [b, v, v, h]
        atten_yi = tf.tile(
            tf.expand_dims(tf.reshape(mean, [-1, v, h_dim]), 1), [1, v, 1, 1]
        )  # [b, v, v, h]
        atten_c = tf.reshape(
            tf.matmul(
                tf.reshape(atten_c, [-1, h_dim]), self.weights["atten_weights_c_in"]
            ),
            [-1, v, v, h_dim],
        )
        atten_yi = tf.reshape(
            tf.matmul(
                tf.reshape(atten_yi, [-1, h_dim]), self.weights["atten_weights_y_in"]
            ),
            [-1, v, v, h_dim],
        )
        atten_mi = tf.nn.sigmoid(tf.add(atten_c, atten_yi) + atten_masks)
        atten_mi = tf.reduce_sum(atten_mi, 2) / tf.tile(
            tf.expand_dims(tf.reduce_sum(self.ops["graph_state_mask_out"], 1), 1),
            [1, v, 1],
        )
        ...
        mean_sampled = (
            tf.reshape(mean, [-1, v, h_dim]) * self.ops["graph_state_mask_out"]
            + atten_mi * z_sampled
        )
    ...
    def generate_cross_entropy(
        ...
        batch_size = tf.shape(
            self.placeholders["initial_node_representation_" + direc]
        )[0]
        # Use latent representation as decoder GNN input
        filtered_z_sampled = self.ops[
            "initial_repre_for_decoder_" + direc_r
        ]  # [b, v, h+h]
        # data needed in this iteration
        incre_adj_mat = self.placeholders["incre_adj_mat_" + direc][
            :, idx, :, :, :
        ]  # [b, e, v, v]
        distance_to_others = self.placeholders["distance_to_others_" + direc][
            :, idx, :
        ]  # [b,v]
        overlapped_edge_features = self.placeholders[
            "overlapped_edge_features_" + direc
        ][:, idx, :]  # [b,v]
        subgraph_freqs = self.placeholders["new_compound_freqs_" + direc][
            :, idx, :
        ]  # [b, v]
        subgraph_edge_freqs = self.placeholders["new_compound_freqs_edge_" + direc][
            :, idx, :, :
        ]  # [b, e, v]
        node_sequence = self.placeholders["node_sequence_" + direc][:, idx, :]  # [b, v]
        node_sequence = tf.expand_dims(node_sequence, axis=2)  # [b, v, 1]
        edge_type_masks = self.placeholders["edge_type_masks_" + direc][
            :, idx, :, :
        ]  # [b, e, v]
        ...
        edge_type_labels = self.placeholders["edge_type_labels_" + direc][
            :, idx, :, :
        ]  # [b, e, v]
        ...
        new_filtered_z_sampled = (
            new_filtered_z_sampled * self.ops["graph_state_mask_" + direc]
        )
        ...
        local_graph_repr_before_expansion = tf.reduce_sum(
            ...
            self.placeholders["node_mask_" + direc], axis=1, keep_dims=True
        )  # [b, h + h]
        ...
        distance_repr = tf.nn.embedding_lookup(
            self.weights["distance_embedding_" + direc_r], distance_to_others
        )  # [b, v, h+h]
        # overlapped edge feature representation
        overlapped_edge_repr = tf.nn.embedding_lookup(
            self.weights["overlapped_edge_weight_" + direc_r], overlapped_edge_features
        )  # [b, v, h+h]
        # concat and reshape.
        ...
        if self.params["use_subgraph_freqs"]:
            ...
            edge_softmax = (
                tf.nn.softmax(edge_logits) * subgraph_freqs
            )  # reweight probabilities by subgraph occurrence
            edge_softmax = edge_softmax / tf.reduce_sum(
                edge_softmax, axis=1, keepdims=True
            )  # Normalise

            # Reweight edge loss by subgraph frequencies
            subgraph_freqs_mask = tf.concat(
                [subgraph_freqs_mask, np.ones((self.params["batch_size"], 1))], axis=1
            )  # [b, v+1]
            ...
            edge_loss = -tf.reduce_sum(
                tf.log(edge_softmax + SMALL_NUMBER) * edge_labels * edge_loss_multi,
                axis=1,
            )
            ...
            edge_type_probs = edge_type_probs / tf.reduce_sum(
                edge_type_probs, axis=1, keepdims=True
            )  # Normalise

            # Reweigh edge type loss by subgraph frequencies
            subgraph_edge_freqs_mask = self.placeholders["edge_type_masks_" + direc][
                :, idx, :, :
            ]
            edge_type_loss_multi = tf.tile(
                tf.reduce_mean(
                    subgraph_edge_freqs * subgraph_edge_freqs_mask + SMALL_NUMBER,
                    axis=1,
                    keep_dims=True,
                )
                / (
                    tf.reduce_sum(
                        subgraph_edge_freqs_mask + SMALL_NUMBER, axis=1, keep_dims=True
                    )
                    + 1.0
                ),
            ...
            edge_type_loss = (
                -edge_type_labels
                * tf.log(edge_type_probs + SMALL_NUMBER)
                * edge_type_loss_multi
            )  # [b, e, v]
        ...
        # Don't reweigh - vanilla cross entropy
            ...
            edge_loss = -tf.reduce_sum(
                tf.log(edge_softmax + SMALL_NUMBER) * edge_labels, axis=1
            )
    ...
    def construct_logit_matrices(self):
        ...
        filtered_z_sampled = tf.concat(
            [self.ops["z_sampled_" + in_direc], latent_node_state], axis=2
        )  # [b, v, h + h]
        ...
        self.ops[
            "edge_type_predictions_" + in_direc
        ] = edge_type_predictions_final.read(0)
    ...
    def construct_loss(self):
        ...
        kl_loss_in = (
            tf.reshape(kl_loss_in, [-1, v, h_dim])
    ...
    def process_raw_graphs(
        ...
        for d, incremental_result_1 in zip(raw_data, incremental_results[1]):
            ...
            bucketed[chosen_bucket_idx].append(
                {
                    "adj_mat_in": graph_to_adj_mat(
                        d["graph_in"],
                        chosen_bucket_size,
                        self.num_edge_types,
                        self.params["tie_fwd_bkwd"],
                    ),
                    "adj_mat_out": graph_to_adj_mat(
                        d["graph_out"],
                        chosen_bucket_size,
                        self.num_edge_types,
                        self.params["tie_fwd_bkwd"],
        ...
        for d in elements:
            ...
            for direc in ["_out"]:
                ...
                incre_adj_mat = incre_adj_mat_to_dense(
                    d["incre_adj_mat" + direc], self.num_edge_types, maximum_vertice_num
                )
                ...
                edge_type_masks = edge_type_masks_to_dense(
                    d["edge_type_masks" + direc],
                    maximum_vertice_num,
                    self.num_edge_types,
                )
                edge_type_labels = edge_type_labels_to_dense(
                    d["edge_type_labels" + direc],
                    maximum_vertice_num,
                    self.num_edge_types,
                )
                edge_masks = edge_masks_to_dense(
                    d["edge_masks" + direc], maximum_vertice_num
                )
                edge_labels = edge_labels_to_dense(
                    d["edge_labels" + direc], maximum_vertice_num
                )

                batch_data["incre_adj_mat" + direc].append(
                    incre_adj_mat
                    + [
                        np.zeros(
                            (
                ...
                batch_data["distance_to_others" + direc].append(
                    distance_to_others
                    + [
                ...
                batch_data["overlapped_edge_features" + direc].append(
                    overlapped_edge_features
                    + [
                        np.zeros((maximum_vertice_num))
                        for _ in range(
                ...
                batch_data["new_compound_freqs" + direc].append(
                    new_compound_freqs
                    + [
                ...
                batch_data["new_compound_freqs_edge" + direc].append(
                    new_compound_freqs_edge
                    + [
                ...
                batch_data["node_sequence" + direc].append(
                    node_sequence
                    + [
                        np.zeros((maximum_vertice_num))
                        for _ in range(
                ...
                batch_data["edge_type_masks" + direc].append(
                    edge_type_masks
                    + [
                ...
                batch_data["edge_masks" + direc].append(
                    edge_masks
                    + [
                        np.zeros((maximum_vertice_num))
                        for _ in range(
                ...
                batch_data["edge_type_labels" + direc].append(
                    edge_type_labels
                    + [
                        np.zeros((self.num_edge_types, maximum_vertice_num))
                        for _ in range(
                ...
                batch_data["edge_labels" + direc].append(
                    edge_labels
                    + [
                        np.zeros((maximum_vertice_num))
                        for _ in range(
    ...
    def get_dynamic_feed_dict(
        ...
        return {
            ...
            self.placeholders["initial_node_representation_in"]: self.pad_annotations(
                [elements["init_in"]]
            ),
            self.placeholders["initial_node_representation_out"]: self.pad_annotations(
                [elements["init_out"]]
            ),
            self.placeholders["node_symbols_out"]: [elements["init_out"]],
            self.placeholders["node_symbols_in"]: [elements["init_in"]],
            self.placeholders["latent_node_symbols_in"]: self.pad_annotations(
                latent_node_symbol
            ),
            self.placeholders["latent_node_symbols_out"]: self.pad_annotations(
                latent_node_symbol
            ),
        ...
        while len(queue) > 0:
            ...
            while True:
                ...
                new_compound_freqs_edge_dense = np.ones(
                    (1, self.num_edge_types, max_n_vertices)
                )  # [1, e, v]
                incre_adj_mat = incre_adj_mat_to_dense(
    ...
    def generate_graph_with_state(
        ...
        sampled_node_symbol = sample_node_symbol(
            predicted_node_symbol_prob, [real_length], self.params["dataset"]
        )[0]  # [v]
        ...
        if not self.params["path_random_order"]:
            ...
            if self.params["try_different_starting"]:
                ...
                starting_point = [
                    random.choice(list(range(real_length)))
                ]  # randomly choose one
        ...
        for idx in starting_point:
            ...
            if (
                self.params["dataset"] == "zinc"
                or self.params["dataset"] == "zinc_phosphorus"
            ) and new_mol is not None:
        ...
        generated_all_smiles.append(
            elements["smiles_in"] + " " + Chem.MolToSmiles(best_mol)
        )
        dump(
            os.path.join(
                self.log_dir,
                "%s_generated_smiles_%s" % (self.run_id, self.params["dataset"]),
            ),
            generated_all_smiles,
        )
        # Progress
    ...
    def compensate_node_length(self, elements, bucket_size):
        ...
        real_length_out = (
            get_graph_length([elements["mask_in"]])[0] + self.params["compensate_num"]
        )
        elements["mask_out"] = [1] * real_length_out + [0] * (
            maximum_length - real_length_out
        )
            ...
            for elements in elements_batch:
                ...
                for _ in range(self.params["number_of_generation_per_valid"]):
                    ...
                    random_normal_states = generate_std_normal(
                        1, maximum_length, self.params["encoding_size"]
                    )  # [1, v, j]
                    random_normal_states_in = generate_std_normal(
                        1, maximum_length, self.params["hidden_size"]
                    )  # [1, v, h]
    ...
    def make_minibatch_iterator(self, data, is_training: bool):
        ...
        for step in range(len(bucket_at_step)):
            ...
            batch_feed_dict = {
                self.placeholders[
                    "initial_node_representation_in"
                ]: initial_representations_in,
                self.placeholders[
                    "initial_node_representation_out"
                ]: initial_representations_out,
                ...
                self.placeholders["iteration_mask_out"]: batch_data[
                    "iteration_mask_out"
                ],
                self.placeholders["incre_adj_mat_out"]: batch_data["incre_adj_mat_out"],
                self.placeholders["distance_to_others_out"]: batch_data[
                    "distance_to_others_out"
                ],
                self.placeholders["node_sequence_out"]: batch_data["node_sequence_out"],
                self.placeholders["edge_type_masks_out"]: batch_data[
                    "edge_type_masks_out"
                ],
                self.placeholders["edge_type_labels_out"]: batch_data[
                    "edge_type_labels_out"
                ],
                ...
                self.placeholders["overlapped_edge_features_out"]: batch_data[
                    "overlapped_edge_features_out"
                ],
                self.placeholders["new_compound_freqs_out"]: batch_data[
                    "new_compound_freqs_out"
                ],
                self.placeholders["new_compound_freqs_edge_out"]: batch_data[
                    "new_compound_freqs_edge_out"
                ],
            }
            bucket_counters[bucket] += 1
