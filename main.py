...
def process_one_protein(cfg: Dict, pdb_file: str, stages: List[Dict], params: RecenterParams) -> None:
    ...
    def run_docking_for_libset(lib_mode: str, stage_prefix: str = "") -> None:
        """Run Phase 6-8 (ligand prep/filtering, multi-stage docking, pH/variant loop,
        summary/cleanup) for a single ligand library view (e.g. all ligands, DUD view, FDA view).

        This helper closes over cfg, paths, pdb_id, variant_token, variant_label,
        cleaned_pdb, receptor_pdbqt, etc. so its signature stays simple.
        stage_prefix is prepended to each stage["name"] (e.g. "dud_stage1").
        lib_mode is just logged for now; future prompts will use it to alter ligand selection.
        """

        logger.info("[libset] mode=%s prefix=%s", lib_mode, stage_prefix)

        stages_local = copy.deepcopy(stages)
        for st in stages_local:
            # Keep the original names if stage_prefix is empty
            if stage_prefix:
                st["name"] = f"{stage_prefix}{st['name']}"
        stages = stages_local
    ...
    # ======================
    # Phase 6–8 – Docking runs per library set
    # ======================
    test_mode_raw = cfg.get("TEST_MODE_ENABLE", "off")
    test_mode = (test_mode_raw or "off").strip().lower()

    if test_mode == "fda+dud":
        # First pass: DUD view (we’ll later hook this to DUD-only ligand selection)
        run_docking_for_libset(lib_mode="dud", stage_prefix="dud_")

        # Second pass: FDA / default view – keep classic stage names
        run_docking_for_libset(lib_mode="fda", stage_prefix="")
    else:
        # Single run, preserving existing behavior
        run_docking_for_libset(lib_mode=test_mode, stage_prefix="")
