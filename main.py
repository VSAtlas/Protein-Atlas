...
def process_one_protein(cfg: Dict, pdb_file: str, stages: List[Dict], params: RecenterParams) -> None:
    ...
    try:
        ...
        mode = (os.environ.get("APO_HOLO_MODE") or "holo").strip().lower()

        if mode == "holo":
            # Keep *only* element-token ions from the retain list
            one = set(rules.one_letter)
            two = set(rules.two_letter)
            keep = {t for t in rules.retain_resnames if (t in one or t in two)}
        else:
            # APO: strip all monoatomic ions (waters/cofactors are multi-atom and unaffected)
            keep = set()

        _strip_monoatomic_ions_inplace(Path(cleaned_pdb), keep_resnames=keep)
        logger.info("%s mode: %s monoatomic ions before pocket detection/Meeko.",
                    "HOLO" if mode == "holo" else "APO",
                    "kept YAML-retained" if mode == "holo" else "stripped all")
    except Exception as e:
...
def main() -> None:
    ...
    start = time.time()
    mode = str(cfg.get("APO_HOLO_MODE", os.environ.get("APO_HOLO_MODE", "holo"))).lower().replace(" ", "")
    variants = ["holo", "apo"] if mode in {"apo_vs_holo", "apovsholo"} else [mode or "holo"]

    for variant in variants:
        cfg_v = dict(cfg)  # shallow copy
        for key in ("OUTPUT_DIR", "DOCKED_DIR", "P2RANK_OUTPUT_DIR", "PDBQT_DIR", "LOGS_DIR"):
            if cfg_v.get(key):
                cfg_v[key] = os.path.join(cfg_v[key], variant.upper())
        os.environ["APO_HOLO_MODE"] = variant  # consumed in activesite.get_atom_rules()

        with tqdm(total=len(pdb_files), desc=f"Processing Proteins ({variant})", unit="protein") as bar:
