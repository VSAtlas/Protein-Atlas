...
from protein_prep.pdb_records import atom_name, line_xyz
    ...
    with open(pdb_path, encoding="utf-8", errors="ignore") as f:
        for line in f:
            if line.startswith(("ATOM", "HETATM")):
                xyz = line_xyz(line)
                el = line[76:78].strip() or atom_name(line)[:1]
                atoms.append((line, el.upper() if el else "UNK", xyz))

    kept: List[str] = []
    heavy_coords = [a[2] for a in atoms if a[1] != "H" and a[2] is not None]
