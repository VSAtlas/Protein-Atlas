...
from protein_prep.pdb_fixer_runtime import log_pre_variant_policy_breadcrumb
...
from protein_prep.pdb_records import line_xyz as _parse_xyz
...
def _maybe_strip_ions(
    ...
    *,
    ...
    for token in allow_tokens:
        ...
        if not token_text:
            continue
        canonical = _normalize_resname(token_text)
        allow_canonical.add(canonical or token_text)
    ...
    if extra_keep:
        for token in extra_keep:
            ...
            if not token_text:
                continue
            canonical = _normalize_resname(token_text)
            extra_canonical.add(canonical or token_text)
    ...
    try:
        log_pre_variant_policy_breadcrumb(path)
    except Exception:
        pass
    ...
    try:
        text: list[str] = path.read_text(encoding="utf-8", errors="ignore").splitlines()
