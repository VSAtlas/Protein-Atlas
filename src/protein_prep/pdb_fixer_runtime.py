...
def summarize_ions(pdb_path: Union[str, Path]) -> dict[str, object]:
    ...
    metal_hits: Dict[str, Set[str]] = {
        token: set() for token in _ION_BREADCRUMB_METAL_ORDER
    }
    simple_hits: Dict[str, Set[str]] = {
        token: set() for token in _ION_BREADCRUMB_SIMPLE_ORDER
    }
...
def log_pre_variant_policy_breadcrumb(pdb_path: Union[str, Path]) -> None:
    ...
    if summary.get("missing"):
        ...
        return
    metals = summary.get("metals")
    simple_ions = summary.get("simple_ions")
    waters = summary.get("waters", 0)
    other_het = summary.get("other_het", 0)
    logging.info(
        "[ions.breadcrumb] stage=pre_variant_policy file=%s metals=%s waters=%d simple_ions=%s other_het=%d",
        short_path,
        _format_breadcrumb_counts(
            metals if isinstance(metals, dict) else {}, _ION_BREADCRUMB_METAL_ORDER
        ),
        int(waters if isinstance(waters, (int, float, str)) else 0),
        _format_breadcrumb_counts(
            simple_ions if isinstance(simple_ions, dict) else {},
            _ION_BREADCRUMB_SIMPLE_ORDER,
        ),
        int(other_het if isinstance(other_het, (int, float, str)) else 0),
    )
...
def _iter_values(payload: Any) -> Iterable[Any]:
    ...
    if isinstance(payload, Iterable):
        ...
        for entry in payload:
            entries.extend(_iter_values(entry))
        return entries
    return [payload]
