...
def _canon_base(output_root: Path, pdb_id: str) -> Path:
    """Canonical per-protein base: processed_pdbs/<RUN_ID>/<PDB>."""
...
def fold_legacy_layout(pdb_id: str, output_root) -> None:
    ...
    Extended: migrate uppercase legacy dirs into canonical tree:
      <PDB>_NOLIG            -> processed_pdbs/<RUN_ID>/<PDB>/nolig
      <PDB>_CLEANED_LIGANDS  -> processed_pdbs/<RUN_ID>/<PDB>/ligands_raw
      <PDB>_nolig(.pdb)      -> processed_pdbs/<RUN_ID>/<PDB>/nolig/<PDB>_nolig_phenix_clean.pdb
    """
    try:
