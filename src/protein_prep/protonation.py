...
from protein_prep.pdb_fixer_runtime import fix_pdb_elements
from protein_prep.propka_wire import pdb2pqr_protonate
...
def _pick_reduce_exe() -> str:
    ...
    # 4) Active Python env, then PATH fallback
    env_reduce = Path(sys.executable).resolve().parent / "reduce"
...
def _maybe_get_target_ph() -> float | None:
    ...
    if not input_pdb_path:
        return None
    return get_target_ph_for_prep(input_pdb_path)
...
def _protonate_with_pdb2pqr_if_available(
    ...
    *,
    ...
    """
    if str(os.environ.get("A2_SKIP_PDB2PQR", "")).strip().lower() in {
        "1",
        "true",
        "yes",
        "y",
        ...
        logger.info(
            "[pdb2pqr] skipped by A2_SKIP_PDB2PQR=1; pH ensemble will handle receptor states"
        )
        return Path(nolig_pdb_path), False, None

    target_ph = get_target_ph_for_prep(nolig_pdb_path)
...
def assign_protonation_states(
    ...
    def run_reduce(in_pdb: str, stage_name: str) -> str:
        if exe is None:
            raise RuntimeError(
                "Reduce tool check failed: executable is unavailable "
                f"(REDUCE_EXE='{REDUCE_EXE}', PHENIX_DIR='{PHENIX_DIR}'). "
                "Install Phenix/Reduce or set REDUCE_EXE in config.txt, then run "
        ...
        cp = run_subprocess_capture([exe_cmd] + reduce_flags + [in_pdb], env=env)
    ...
    if exe is None:
        ...
        if _FALLBACK_ADDH:
            ...
            try:
                ...
                logging.warning(
                    "[protonate] OpenBabel fallback failed/timeout; copying input. note=%s",
                    str(e)[:240],
                )
            ...
            try:
                ...
                def run_reduce_noflip(in_pdb: str, stage_name: str) -> str:
                    ...
                    cp = run_subprocess_capture(
                        [exe_cmd] + reduce_flags_noflip + [in_pdb],
                        cwd=exe_dir,
                        env=env,
                    )
                    with open(output_pdb, "w", encoding="utf-8") as out:
