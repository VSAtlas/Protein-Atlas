...
def load_microstate_registry(
    ...
    registry: dict[str, Any] = default_registry.copy()
    ...
    microstate_index: Dict[str, dict[str, Any]] = {}
...
def _manifest_pdbqt_fallback_paths(library_out_dir: Path) -> List[Path]:
...
def enumerate_ligands_for_docking(
    ...
    *,
    ...
    from prep_ligands.prep_ligands_bulk_meeko import (
    ...
    if (not registry_path.exists()) or not (registry.get("microstates") or []):
        ...
        pdbqt_paths = _manifest_pdbqt_fallback_paths(library_out_dir)
        if pdbqt_paths:
            logger.info(
    ...
    if not result:
        ...
        if pdbqt_paths:
            logger.warning(
                "enumerate_ligands_for_docking: microstate registry produced zero usable PDBQT(s); manifest fallback returning %d PDBQT(s) (ignoring requested_ph_values=%s)",
                len(pdbqt_paths),
                "any" if requested_set is None else sorted(requested_set),
            )
            return pdbqt_paths
