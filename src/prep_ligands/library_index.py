...
class LibraryIndex:
    ...
    DEFAULT_PARENT_MANIFEST_FILENAME = "_libraries_manifest.json"
    ...
    def _child_dir_mtimes(root: Path) -> dict[str, float]:
        """
        Lightweight freshness signal for library roots.

        We track direct child directory mtimes (e.g., actives/decoys/microstates)
        to detect stale manifests even when only nested ligand trees change.
        """
        out: dict[str, float] = {}
        try:
    ...
    @staticmethod
    def _manifest_references_valid(
        root: Path,
        *,
        entries: dict[str, str],
        filenames: dict[str, str],
    ) -> bool:
        """Validate manifest references without relying on directory mtimes."""
        root = Path(root)
        try:
        ...
        rel_values = list(entries.values()) + list(filenames.values())
        ...
        for rel in rel_values:
            ...
            try:
                ...
                if not resolved.is_file():
                    return False
                if int(resolved.stat().st_size) <= 0:
                    return False
            except Exception:
                return False
        return True
    ...
    def load(self, roots: Sequence[Path]) -> None:
        ...
        ordered = self._expand_roots_via_parent_manifest(ordered)
        self._roots = ordered
        ...
        if data and isinstance(data, dict):
            ...
            if self._strict_mtime_check:
                if isinstance(raw_entries, dict) and isinstance(raw_files, dict):
                    ...
                    if self._manifest_references_valid(
                        root,
                        entries=entries,
                        filenames=filenames,
                    ):
    ...
    def _build_manifest(
        ...
        try:
            ...
            with open(manifest_path, "w", encoding="utf-8") as fh:
                payload = {
                    "mtime": mtime,
                    "child_dir_mtimes": self._child_dir_mtimes(root),
                    "entries": entries,
                    "filenames": filenames,
                }
                if orjson is not None:
                    fh.write(orjson.dumps(payload).decode("utf-8"))
                else:
    ...
    def write_manifest_for_root(
        ...
        try:
            manifest_path.parent.mkdir(parents=True, exist_ok=True)
            with open(manifest_path, "w", encoding="utf-8") as fh:
                payload = {"mtime": mtime, "entries": entries, "filenames": filenames}
                payload["child_dir_mtimes"] = self._child_dir_mtimes(root)
                if orjson is not None:
                    fh.write(orjson.dumps(payload).decode("utf-8"))
                else:
                    json.dump(payload, fh, indent=2, sort_keys=True)
    ...
    def _expand_roots_via_parent_manifest(self, roots: Sequence[Path]) -> list[Path]:
        """Expand roots that declare per-library children via a parent manifest."""
        expanded: list[Path] = []
        seen: set[str] = set()
        for root in roots:
            parent_manifest = root / self.DEFAULT_PARENT_MANIFEST_FILENAME
            ...
            try:
                raw = parent_manifest.read_text(encoding="utf-8")
                parsed = orjson.loads(raw) if orjson is not None else json.loads(raw)
                mapping = parsed.get("libraries") if isinstance(parsed, dict) else None
                if mapping is None and isinstance(parsed, dict):
                    mapping = parsed
                if not isinstance(mapping, dict):
                    raise ValueError("parent manifest must be a mapping")
                children: list[Path] = []
                ...
                if children:
                    ...
                    self._logger.info(
                        "[lib-index.parent] root=%s parent=%s children=%d",
                        str(root),
                        str(parent_manifest),
                        len(children),
                    )
                    continue
                raise ValueError("no valid child directories found")
            except Exception as exc:
                self._logger.warning(
                    "[lib-index.parent] Failed to load parent manifest %s: %s; falling back to root",
                    str(parent_manifest),
                    exc,
                )
            key = str(root.resolve())
    ...
    def _acquire_manifest_lock(self, root: Path) -> Iterator[bool]:
        """
        Try to acquire a best-effort lock for manifest rebuild.
        Yields True if we hold the lock, False otherwise.
        """
        lock_path = root / f"{self._manifest_filename}.lock"
        ...
        try:
            ...
            self._logger.info(
                "[lib-index.build] root=%s Another process is rebuilding manifest, skipping rebuild",
                str(root),
            )
            yield False
        except Exception:
