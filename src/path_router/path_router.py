...
def expand_variants(mode: Optional[str]) -> list[Optional[str]]:
    ...
    logger.debug("[router.debug] expand_variants.in=%r -> %r", mode, expanded)
    return expanded
...
def _ph_manifest_candidates(pdb_id: str, variant: Optional[str]) -> list[Path]:
    ...
    Return the only two allowed search locations for ensemble.json, in order:
      1) processed_pdbs/<RUN_ID>/<PDB>/<VARIANT>/receptor/ph_ensemble/ensemble.json
         when variant is given
      2) processed_pdbs/<RUN_ID>/<PDB>/receptor/ph_ensemble/ensemble.json
    """
...
def ph_ensemble_dir(
    ...
    If variant in {APO, HOLO}:
      processed_pdbs/<RUN_ID>/<PDB>/<VARIANT>/receptor/ph_ensemble/
    Else:
      processed_pdbs/<RUN_ID>/<PDB>/receptor/ph_ensemble/
    """
    roots = _ensure_router_roots()
    token = _norm_pdb_id(pdb_id)
    base = roots.processed / token
...
def _load_router_roots() -> RouterRoots:
    ...
    processed = (
        Path(out_raw).expanduser() if out_raw else output_root(overall, "processed_pdbs")
    )
    processed = run_scoped_root(processed, run_id)

    dock_raw = expanded.get("DOCKED_DIR")
    docked = Path(dock_raw).expanduser() if dock_raw else output_root(overall, "docked")
    if run_id:
        try:
    ...
    configs = Path(cfg_raw).expanduser() if cfg_raw else output_root(overall, "configs")
...
def _set_router_roots(
    ...
    _ROUTER_ROOTS = RouterRoots(
        ...
        if configs
        else output_root(Path(overall).expanduser(), "configs"),
    )
...
def _ensure_router_roots() -> RouterRoots:
    ...
    try:
        ...
        _ROUTER_ROOTS = RouterRoots(
            overall=cwd,
            processed=output_root(cwd, "processed_pdbs"),
            docked=output_root(cwd, "docked"),
            configs=output_root(cwd, "configs"),
        )
...
def run_logs_dir(cfg: Optional[Dict[str, object]] = None) -> Path:
    ...
    logger = logging.getLogger("path_router")
    logger.debug("[router.debug] kind=run_logs_dir path=%s", log_dir)
    return log_dir
...
def raw_dir_v(pdb_id: str, variant: Optional[str] = None) -> Path:
    ...
    path = base / "raw"
    logger = logging.getLogger("path_router")
    logger.debug("[router.debug] kind=raw variant=%s path=%s", v or "None", path)
    return path
...
def nolig_dir_v(pdb_id: str, variant: Optional[str] = None) -> Path:
    ...
    path = base / "nolig"
    logger = logging.getLogger("path_router")
    logger.debug("[router.debug] kind=nolig variant=%s path=%s", v or "None", path)
    return path
...
def work_dir_v(pdb_id: str, variant: Optional[str] = None) -> Path:
    ...
    path = base / "work"
    logger = logging.getLogger("path_router")
    logger.debug("[router.debug] kind=work variant=%s path=%s", v or "None", path)
    return path
...
class Paths:
    ...
    processed_root: Path  # OUTPUT_DIR (processed_pdbs/<RUN_ID>/)
    ...
    root_pdb_dir: Path  # processed_pdbs/<RUN_ID>/<PDB>/

    # Shared subfolders under processed_pdbs/<RUN_ID>/<PDB>/
    raw_dir: Path  # processed_pdbs/<RUN_ID>/<PDB>/raw/          # copies/ingest
    ligands_raw_dir: (
        Path  # processed_pdbs/<RUN_ID>/<PDB>/ligands_raw/  # extracted crystal ligands (*.pdb)
    )
    nolig_dir: Path  # processed_pdbs/<RUN_ID>/<PDB>/nolig/        # <PDB>_nolig.pdb intermediate
    work_dir: Path  # processed_pdbs/<RUN_ID>/<PDB>/work/         # scratch & intermediates

    # Inputs
    input_pdb_path: Path  # input_pdbs/<pdb_file>              # source PDB to process

    # Shared expected filenames (variant-agnostic)
    nolig_pdb_path: Path  # processed_pdbs/<RUN_ID>/<PDB>/nolig/<PDB>_nolig.pdb

    # ---------- Variant-aware receptor paths (back-compat with variant=None) ----------
    def receptor_dir(self, variant: Optional[str]) -> Path:
        """
        If variant in {APO,HOLO}:
          Dir: processed_pdbs/<RUN_ID>/<PDB>/<VARIANT>/receptor/
          Files: <PDB>_cleaned.pdb, <PDB>.pdbqt, optional pH-tokenized PDBQTs.
        If variant is None (legacy):
          Dir: processed_pdbs/<RUN_ID>/<PDB>/receptor/
          Files: <PDB>_cleaned.pdb, <PDB>.pdbqt, optional pH-tokenized PDBQTs.
        """
    ...
    def receptor_cleaned_pdb(self, variant: Optional[str]) -> Path:
        ...
        File:
          * With variant:
            processed_pdbs/<RUN_ID>/<PDB>/<VARIANT>/receptor/<PDB>_cleaned.pdb
          * No variant:
            processed_pdbs/<RUN_ID>/<PDB>/receptor/<PDB>_cleaned.pdb
        """
    ...
    def ligand_output_dir(self) -> Path:
        """
        Dir: processed_pdbs/<RUN_ID>/<PDB>/ligands_raw/
    ...
    def legacy_extracted_ligands_dir(self) -> Optional[Path]:
        ...
        Note: canonical location is processed_pdbs/<RUN_ID>/<PDB>/ligands_raw/.
        """
...
# ---------------------------
def make_paths(cfg: Mapping, base_id: str, pdb_file: str) -> Paths:
    """
    Build per-PDB router from config (no global state).

    Ensures canonical per-PDB directories exist immediately:
      processed_pdbs/<RUN_ID>/<PDB>/{raw, ligands_raw, nolig, work}/
    ...
    processed_root = Path(cfg["OUTPUT_DIR"])  # processed_pdbs[/<RUN_ID>]/
    docked_root = Path(cfg["DOCKED_DIR"])  # docked/
    run_id = str(cfg.get("RUN_ID") or "").strip()
    if run_id:
        os.environ["ATLAS_RUN_ID"] = run_id
        processed_root = run_scoped_root(processed_root, run_id)
    ...
    # Configs root: default under outputs/configs unless CONFIGS_DIR is supplied.
    configs_root = Path(cfg.get("CONFIGS_DIR", output_root(over_root, "configs")))

    # Canonical per-PDB root and shared subfolders
    root_pdb_dir = processed_root / pdb_id  # processed_pdbs/<RUN_ID>/<PDB>/
    raw_dir = root_pdb_dir / "raw"  # processed_pdbs/<RUN_ID>/<PDB>/raw/
    lig_raw_dir = root_pdb_dir / "ligands_raw"  # processed_pdbs/<RUN_ID>/<PDB>/ligands_raw/
    nolig_dir = root_pdb_dir / "nolig"  # processed_pdbs/<RUN_ID>/<PDB>/nolig/
    work_dir = root_pdb_dir / "work"  # processed_pdbs/<RUN_ID>/<PDB>/work/
    ...
    )  # processed_pdbs/<RUN_ID>/<PDB>/nolig/<PDB>_nolig.pdb
