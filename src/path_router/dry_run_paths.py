...
def make(cfg, pdb_id="1T46", pdb_file="1T46.pdb"):
    return Paths(
        ...
        configs_root=Path(
            cfg.get(
                "CONFIGS_DIR",
                output_root(Path(cfg["OVERALL_DIR"]), "configs"),
            )
        ),
        ...
        nolig_pdb_path=Path(cfg["OUTPUT_DIR"])
        / pdb_id
        / "nolig"
        / f"{pdb_id}_nolig.pdb",
    )
...
show(
    "receptor_pdbqt", paths.receptor_pdbqt("APO", "pH8_0+8_1+8_2")
)  # .../1T46_pH8_0+8_1+8_2.pdbqt
