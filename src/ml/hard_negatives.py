        ...
        rows.append(
            {
                ...
                "LogP": float(Crippen.MolLogP(mol)),  # type: ignore[attr-defined]
                "TPSA": float(rdMolDescriptors.CalcTPSA(mol)),
                "HBD": float(Lipinski.NumHDonors(mol)),  # type: ignore[attr-defined]
                "HBA": float(Lipinski.NumHAcceptors(mol)),  # type: ignore[attr-defined]
            }
        )
...
def _compute_morgan_fps(smiles: pd.Series, n_bits: int = 2048) -> dict[int, Any]:
    ...
    for idx, smi in smiles.items():
        ...
        if mol is None:
            continue
        fps[int(idx)] = AllChem.GetMorganFingerprintAsBitVect(  # type: ignore[attr-defined]
            mol, radius=2, nBits=int(n_bits)
        )
    return fps
