...
try:
    from post_docking.rescoring.rescore_reranker import find_consensus_csv
except Exception:
...
def resolve_roots(
    ...
    docked_override = getattr(args, "docked_root", None)
    post_docked_override = getattr(args, "post_docked_root", None)
    docked_root = (
        Path(docked_override).resolve()
        if docked_override
        else output_root(repo_root, "docked")
    )
    post_docked_root = (
        Path(post_docked_override).resolve()
        if post_docked_override
        else output_root(repo_root, "post_docked")
    )
    cfg_map = cfg or {}
...
def _select_best_pose(
    ...
    if any(p.get("cnn_score") is not None for p in poses):
        scores = [
            cast(float, p["cnn_score"]) for p in poses if p.get("cnn_score") is not None
        ]
...
def main() -> int:
    ...
    repo_root, docked_root, post_root, processed_root = resolve_roots(args, cfg)
    ...
    for combo in sorted(combos):
        ...
        consensus_csv = (
            find_consensus_csv(dock_combo_dir) if find_consensus_csv is not None else None
        )
