...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--decoy-prefix",
        default=None,
        help="Override decoy prefix (default: config DECOY_PREFIX/DUD_PREFIX or inferred from TEST_MODE_ENABLE)",
    )
    parser.add_argument(
        "--workers",
        type=int,
...
def discover_tasks(
    ...
    *,
    ...
    for value in prefixes:
        ledock_decoy_dirs |= set(
            _scorch_prefix_mod.decoy_engine_stage_dirs(
                "ledock",
                (1, 2, 3),
                decoy_prefix=value,
            )
        )
        ledock_decoy_legacy_dirs |= set(
            _scorch_prefix_mod.decoy_engine_stage_dirs_legacy(
                "ledock",
                (1, 2, 3),
                decoy_prefix=value,
            )
        )
        dock6_decoy_dirs |= set(
            _scorch_prefix_mod.decoy_engine_stage_dirs(
                "dock6",
                (1, 2, 3),
                decoy_prefix=value,
