...
from config.value_access import cfg_get, to_bool, to_float, to_int
...
def _to_float(val: object, default: float) -> float:
    return float(to_float(val, default=default))
...
def _select_cpptraj_runner(
    ...
    del logger
    selection = select_ambertools_tool(cfg, "cpptraj")
...
def _select_sander_runner(
    ...
    for tool in tool_order:
        try:
            selection = select_ambertools_tool(cfg, tool, prefix_exe_style="absolute")
            return selection.runner, selection.exe, selection.source, selection.prefix
...
def _run_md_qc_cpptraj(
    *,
    ...
    if not _to_bool(_cfg_get(cfg, "MMGBSA_QC_RUN_CPPTRAJ", True), default=True):
        return {"ok": False, "skipped": True, "reason": "disabled"}

    ligand_mask = str(_cfg_get(cfg, "MMGBSA_QC_LIGAND_MASK", ":MOL") or ":MOL")
    qc_dir = md_dir / "qc"
    qc_dir.mkdir(parents=True, exist_ok=True)
    input_path = qc_dir / "cpptraj_qc.in"
    log_path = qc_dir / "cpptraj_qc.log"
    ...
    try:
        runner, cpptraj_bin, source = _select_cpptraj_runner(logger, cfg)
    except Exception as exc:
        _log(
            logger,
            "WARNING",
            {"replicate": replicate_index, "reason": type(exc).__name__, "detail": exc},
            "md_qc_cpptraj_unavailable",
            component=_COMPONENT_MD,
        )
        return {"ok": False, "skipped": True, "reason": str(exc)}

    cmd = list(runner) + [cpptraj_bin, "-i", str(input_path)]
    with log_path.open("w", encoding="utf-8") as handle:
        proc = subprocess.run(
    ...
    _log(
        ...
        "md_qc_cpptraj_done" if ok else "md_qc_cpptraj_failed",
        component=_COMPONENT_MD,
    )
    return {
        "ok": ok,
...
def run_implicit_md(
    ...
    solvent_model = (
        str(_cfg_get(cfg, "MMGBSA_MD_SOLVENT_MODEL", "implicit") or "implicit")
        .strip()
        .lower()
    )
    if solvent_model == "explicit":
        explicit_traj = str(
            _cfg_get(cfg, "MMGBSA_EXPLICIT_TRAJ_PATH", "")
            or _cfg_get(cfg, "MMGBSA_TRAJIN_PATH", "")
            or ""
        ).strip()
        if not explicit_traj:
            raise NotImplementedError(
                "MMGBSA_MD_SOLVENT_MODEL=explicit requires MMGBSA_EXPLICIT_TRAJ_PATH "
                "or MMGBSA_TRAJIN_PATH pointing to a pre-equilibrated stripped trajectory"
            )
        explicit_path = Path(explicit_traj).expanduser()
        if not explicit_path.exists() or explicit_path.stat().st_size == 0:
            raise FileNotFoundError(f"explicit solvent trajectory not found: {explicit_path}")
        md_dir.mkdir(parents=True, exist_ok=True)
        qc_result = _run_md_qc_cpptraj(
            logger=logger,
            cfg=cfg,
            prmtop_path=prmtop_path,
            traj_path=explicit_path,
            md_dir=md_dir,
        ...
        return {
            ...
            "engine": "external_explicit",
            "mpi_ranks": 1,
            "mpi_launcher": "external",
            "qc": qc_result,
        }
    if solvent_model != "implicit":
        raise ValueError(
            f"MMGBSA_MD_SOLVENT_MODEL must be implicit or explicit (got {solvent_model})"
        )
    ...
    return _result({"ok": True, "run": True, "skipped": False, "qc": qc_result})
