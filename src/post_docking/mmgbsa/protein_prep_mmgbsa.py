...
from post_docking.mmgbsa._atomic_io import tmp_path, write_text_atomic
from protein_prep.pdb_records import line_xyz as _parse_coords
from protein_prep.pdb_records import residue_key_text as _residue_key
...
from post_docking.mmgbsa.prep_for_mmgbsa import prep_mmgbsa_from_sdf
...
def _strip_receptor_h_for_leap(
    ...
    kept_lines: List[str] = []
    stripped_hoh_residues: set[str] = set()
    removed_h = 0
    ...
    with receptor_path.open("r", encoding="utf-8", errors="ignore") as handle:
        for line in handle:
            if line.startswith("ATOM") or line.startswith("HETATM"):
                resn_raw = line[17:20].strip().upper()
                if resn_raw == "HOH":
                    stripped_hoh_residues.add(_residue_key(line))
    ...
    return out_path, {
        "strip_all_h": True,
        "removed_h": removed_h,
        "hoh_residue_count": len(stripped_hoh_residues),
    }
...
def _select_tleap_runner(logger: logging.Logger) -> Tuple[List[str], str, str]:
    del logger
    cfg = load_config()
    selection = select_ambertools_tool(cfg, "tleap")
...
def _run_tleap_impl(leap_file_path: str, work_dir: str, force: bool = False) -> dict:
    ...
    for line in reversed(log_text.splitlines()):
        if "Exiting LEaP:" not in line or "Errors =" not in line:
            continue
        try:
            log_error_count = int(line.rsplit("Errors =", 1)[1].split(";", 1)[0])
        except Exception:
            log_error_count = 1
        break
    leap_log_failed = log_error_count > 0 or "Parameter file was not saved" in log_text
    result["leap_log_errors"] = log_error_count
    result["ok"] = proc.returncode == 0 and not leap_log_failed

    if proc.returncode != 0 or leap_log_failed:
        _log_leap(
            logger,
            "ERROR",
            {
                "returncode": proc.returncode,
                "leap_log_errors": log_error_count,
                "log": log_path,
            },
            "tleap_failed",
        )
        raise RuntimeError(
            f"tleap failed (code={proc.returncode}, log_errors={log_error_count}); "
            f"see {log_path}"
        )
...
def prep_mmgbsa_receptor(
    ...
    if strip_nterm_h or insert_ter_on_break:
        ...
        raw_stripped_res = sanitize_info.get("nterm_h_stripped_residues")
        stripped_res = (
            [str(residue) for residue in raw_stripped_res]
            if isinstance(raw_stripped_res, list)
            else []
        )
        if stripped_res:
...
def prep_mmgbsa_receptor_and_topologies(
    ...
    hoh_residue_count_raw = strip_info.get("hoh_residue_count", 0)
    hoh_residue_count = (
        int(hoh_residue_count_raw)
        if isinstance(hoh_residue_count_raw, (int, float, str, bytes, bytearray))
        else 0
    )
    _log_leap_prep(
        logger,
