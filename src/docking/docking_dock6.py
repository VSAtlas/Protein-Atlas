...
def _parse_dock6_ranked_mol2(
    ...
    for idx, (pdbqt_path, _mol2_path) in enumerate(ligand_pairs):
        ...
        if idx < len(parsed):
            parsed_score = parsed[idx][1]

        metrics[pdbqt_path] = {
            "grid_score": parsed_score,
            "n_poses": 1 if parsed_score is not None else 0,
            "valid": parsed_score is not None,
            "reason": None if parsed_score is not None else "missing_from_ranked",
