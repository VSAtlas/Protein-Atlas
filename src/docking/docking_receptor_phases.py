...
from protein_prep import automate_protein_prep as protein_prep
from docking.apo_holo_runtime import (
...
def _phase2_to4_receptor_and_center(
    ...
    # Controls-first policy: do not run active-site/P2Rank here.
    # Active-site detection is deferred to phase5b fallback only if
    # control centering cannot produce a usable center/box.
    logger.info(
        "[active-site] deferred_until_control_fallback pdb=%s variant=%s",
        paths.pdb_id,
        variant_label,
    )

    try:
        from protein_prep import automate_protein_prep as _auto_prep_mod
    ...
    if box_size is not None:
        ...
        box_size = cast(
            ...
            tuple(min(box_cap, float(s)) for s in box_size[:3]),
        )
