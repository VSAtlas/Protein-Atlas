...
from chemdb.decoy_control import decoy_role_token
...
def _consensus_sort_key(row: Dict[str, Any]) -> float:
    try:
        score = float(row.get("consensus_score", float("-inf")))
    except Exception:
        return float("-inf")
...
def _is_decoy_run_mode(run_mode: Optional[str]) -> bool:
    return decoy_role_token(run_mode)
...
def compute_consensus_for_variant_ph(
    ...
    *,
    ...
    effective_run_mode = (
        run_mode if run_mode else ("fda" if csv_prefix == "" else csv_prefix.rstrip("_"))
    )
    fda_prefixes = _library_prefixes(cfg, "FDA_PREFIXES", FDA_PREFIX_DEFAULTS)
    decoy_prefixes = _library_prefixes(cfg, "DECOY_PREFIXES", DECOY_PREFIX_DEFAULTS)
    ...
    for lig in ligand_ids:
        ...
        library_role = _infer_library(ligand_display, fda_prefixes, decoy_prefixes)
        row_is_decoy = library_role == "DECOY"
        out_rows.append(
            {
                "run_id": run_id,
                ...
                "library": effective_run_mode,
                "is_decoy": "1" if row_is_decoy else "0",
    ...
    for row in out_rows:
        row_consensus_score = row.get("consensus_score")
        if not _to_bool_flag(row.get("is_decoy")):
            continue
        if not isinstance(row_consensus_score, (int, float)):
            continue
    ...
    if decoy_scores:
        ...
        if sigma_decoy > 0 and math.isfinite(mu_decoy) and math.isfinite(sigma_decoy):
            ...
            log.warning(
                "[z-score.skip] pdb=%s variant=%s ph=%s reason=sigma_zero_or_nonfinite n_decoys=%d",
                paths.pdb_id,
                variant_label,
                ph_label or "",
                len(decoy_scores),
        ...
        log.info(
            "[z-score.skip] pdb=%s variant=%s ph=%s reason=no_decoy_scores n_decoys=0",
            paths.pdb_id,
            variant_label,
            ph_label or "",
        )

    out_path = variant_root / f"{csv_prefix}consensus_docking_scores.csv"
    if out_rows:
        out_rows.sort(key=_consensus_sort_key, reverse=True)
