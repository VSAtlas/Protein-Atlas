...
def _set_prepare_receptor_provenance(value: str) -> None:
    setattr(prepare_receptor, "last_provenance", value)
...
def _bool_from_config(value: object, default: bool = False) -> bool:
    ...
    if token in {"y", "yes", "t", "true", "on", "1"}:
        return True
    if token in {"n", "no", "f", "false", "off", "0"}:
        return False
    return default
...
def prepare_receptor(
    ...
    _set_prepare_receptor_provenance("unknown")
    force_reprocess = _bool_from_config(cfg.get("FORCE_REPROCESS", False))
    ...
    cached_prep_required = _bool_from_config(
        cfg.get("_BENCH_CACHED_PREP_REQUIRED", False)
    )
    ...
    def _build_ph_ensemble(cleaned_path: str) -> Optional[str]:
        ...
        try:
            ...
            prev_cfg = None
        try:
            from docking import ph_ensemble_runtime as ph_ensemble
    ...
    if (
        ...
        try:
            if bool(
                ...
                message = "Receptor sanity check failed (cached receptor)."
                logger.warning(message)
                _set_prepare_receptor_provenance("cache_reuse_failed")
                if cached_prep_required:
        ...
        _set_prepare_receptor_provenance("cache_reuse")
        return cleaned_norm, receptor_norm

    if cached_prep_required:
        missing = []
        if not cleaned_pdb_path.exists():
            missing.append(str(cleaned_pdb_path))
        if not receptor_pdbqt_path.exists():
            missing.append(str(receptor_pdbqt_path))
        message = (
            "[bench cached-prep-required] missing prepared receptor artifacts "
            f"for {paths.pdb_id} variant={variant or 'legacy'}; "
            f"FORCE_REPROCESS={force_reprocess}; missing={missing}"
        )
        logger.error(message)
        _set_prepare_receptor_provenance("cache_required_missing")
        raise RuntimeError(message)
    ...
    try:
        ...
        _set_prepare_receptor_provenance("clean_failed")
        return None, None
        ...
        if context_ph_values:
            ...
            for p in context_ph_values:
                ...
                for delta in (-1.0, 0.0, +1.0):
                    ...
                    ligand_window.add(round(v, 1))

            ligand_ph_values = sorted(ligand_window)
            if ligand_ph_values:
                logger.info(
                    "[ph_ligand] mode=%s context_pH=%s ligand_pH_window=%s",
                    mode,
    ...
    _set_prepare_receptor_provenance(provenance)
