...
from cli.run_manifest_runtime import emit_pocket_detection_event, PocketDetectionEvent
from docking.druggability_policy import decide_engine_policy
from docking.docking_gnina import run_gnina_for_stage
from docking.docking_gnina_support import should_run_gnina_for_target
...
def _control_stage_info(cfg: Dict[str, Any]) -> Dict[str, Any]:
    info: Dict[str, Any] = {
    ...
    if cfg.get("FAST_MODE") and _cfg_bool(
        cfg,
...
def _control_centers_by_ph(
    ...
    *,
    ...
    if ph_override:
        ph_tags = [None if ph_override.lower() == "base" else ph_override]
    elif bool(cfg.get("PH_ENSEMBLE", False)):
        ph_tags = [
            str(tag).strip()
...
def run_control_docking_multi_engine(
    ...
    *,
    ...
    stage_info = _control_stage_info(cfg)
    if cfg.get("FAST_MODE") and not _cfg_bool(
        cfg,
        "CTRL_REDOCK_RESPECT_FAST_MODE",
        False,
    ):
        logger.info(
            "[control-multi] fast_mode_ignored_for_controls=true exhaustiveness=%s num_modes=%s",
            stage_info.get("exhaustiveness"),
            stage_info.get("num_modes"),
        )
    ...
    for ph_label in ph_labels:
        ...
        for lig_path in control_paths:
            ...
            try:
                ...
                with acquire_global_cores(
                    cfg,
                    cores=max(1, int(vina_threads)),
                    min_cores=1,
                    priority=3,
                    task_id=f"{paths.pdb_id}:ctrl:{lig_base}",
        ...
        if "gnina" in engines:
            ...
            for lig_path in control_paths:
                ...
                if score is None:
                    sc_raw = (
                        gnina_scores.get(str(lig_path))
                        or gnina_scores.get(Path(lig_path).as_posix())
                    )
