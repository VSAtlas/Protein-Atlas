...
def annotate_ledock_fda_long_csv_with_z_scores_vs_decoys(
    cfg: Dict[str, Any],
    pdb_id: str,
    ...
    *,
    ...
    """
    Annotate LeDock FDA long CSV with z-scores vs decoys, mirroring Vina/GNINA helpers.
    """
    ...
    if not dud_csv.exists() or not fda_csv.exists():
        if logger:
            logger.info(
                "[ledock.z-score.skip] pdb_id=%s ph=%s reason=missing_csv dud=%s fda=%s",
                pdb_id,
                ph_label or "base",
                str(dud_csv),
                str(fda_csv),
    ...
    if (
        ...
        if logger:
            logger.info(
                "[ledock.z-score.skip] pdb_id=%s ph=%s reason=degenerate_stats n=%s mu=%s sigma=%s",
                pdb_id,
                ph_label or "base",
                n_decoys,
                mu,
    ...
    best["ledock_z_vs_decoys"] = (mu - best["best_score"]) / sigma
    z_map = dict(zip(best[lig_col], best["ledock_z_vs_decoys"]))
    df["ledock_z_vs_decoys"] = df[lig_col].map(z_map)
    # Legacy compatibility alias.
    df["ledock_t_vs_decoys"] = df["ledock_z_vs_decoys"]

    df.to_csv(fda_csv, index=False)
    if logger:
        logger.info(
            "[ledock.z-score.ok] pdb_id=%s ph=%s n_decoys=%s mean=%.3f std=%.3f out=%s",
            pdb_id,
            ph_label or "base",
            n_decoys,
            mu,
...
def annotate_ledock_fda_long_csv_with_t_scores_vs_decoys(
    ...
    *,
    ...
    return annotate_ledock_fda_long_csv_with_z_scores_vs_decoys(
        cfg,
        pdb_id,
        ph_label=ph_label,
...
) -> Tuple[Dict[Path, Optional[float]], Dict[Path, Dict[str, Any]], Dict[str, Any]]:
