...
import hashlib
from protein_prep.propka_wire import apply_propka_states
...
def build_ph_ensemble(
    ...
    dedup_members: list[dict[str, Any]] = []
    canonical_phs: list[float] = []

    for sha, grp in groups.items():
        # canonical = lowest numeric pH
        grp_sorted = sorted(grp, key=lambda d: float(cast(float | str, d["ph"])))
        canonical = grp_sorted[0]
        phs = sorted(float(cast(float | str, x["ph"])) for x in grp_sorted)
        ...
        collapsed_path = Path(cast(str, canonical["pdbqt"])).parent / collapsed_name

        # rename only if there is more than one in the equivalence class
        out_pdbqt_path = Path(cast(str, canonical["pdbqt"]))
        if len(grp_sorted) > 1:
            ...
            elog.info(
                ...
                float(cast(float | str, canonical["ph"])),
            )
        else:
            elog.info(
        ...
        dedup_members.append(
            {
                "ph": float(cast(float | str, canonical["ph"])),
                "ph_equiv": phs,
                "tag": canonical["tag"],
        ...
        canonical_phs.append(float(cast(float | str, canonical["ph"])))
