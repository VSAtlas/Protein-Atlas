...
from docking.checkpoints import (
    ...
    *,
    ...
    while i < len(stages_for_run):
        ...
        stage_noncontrols = [
            ...
            if norm(ligand_path) not in control_norms_for_run
            and Path(ligand_path).stem.split("_stage")[0].lower()
            not in ctrl_stems_lower
        ]
        stage_ligands = (
        ...
        stage_ligands_for_audit = [norm(ligand_path) for ligand_path in stage_ligands]
        ...
        try:
            if cfg.get("ADAPTIVE_SHRINK_ENABLE", True) and validated:
                ...
                if (med is not None) and (
                    ...
                    new_box = (
                        max(min_box, box_size[0] - dec),
                        max(min_box, box_size[1] - dec),
                        max(min_box, box_size[2] - dec),
                    )
                    if new_box != box_size:
                        logger.info(
