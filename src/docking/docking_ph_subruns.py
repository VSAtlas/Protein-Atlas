...
def resolve_ph_tags_and_root(
    *,
    ...
    legacy_mode: bool,
) -> Tuple[List[Optional[str]], Optional[Path], Optional[str], Optional[str]]:
    """
    ...
    """
    ph_enabled = _to_bool(cfg.get("PH_ENSEMBLE", False))
    ...
    if ph_override:
        ph_tags: List[Optional[str]] = [ph_override]
    elif not ph_enabled:
        ph_tags = [None]
    else:
    ...
    logger.info(
        ...
        run_mode or "None",
        ",".join(str(tag or "base") for tag in ph_tags) if ph_tags else "(none)",
    ...
    if ph_enabled:
        prewarm_ph_ligand_microstates(
            cfg,
            [str(tag) for tag in ph_tags if tag],
            ph_ligand_root,
        )
