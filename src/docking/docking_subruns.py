...
from docking.apo_holo_runtime import _record_apo_holo_usage
...
def _run_scoped_receptor_fallback(
    cfg: Dict[str, Any],
    pdb_id: str,
    variant_token: Optional[str],
    ph_label: Optional[str],
) -> Optional[Path]:
    """Resolve receptors written under OUTPUT_DIR/<RUN_ID>/... when router roots are unscoped."""
    run_id = str(cfg.get("RUN_ID") or os.environ.get("ATLAS_RUN_ID") or "").strip()
    output_root_raw = cfg.get("OUTPUT_DIR") or os.environ.get("OUTPUT_DIR")
    if not run_id or not output_root_raw:
        return None

    root = Path(str(output_root_raw))
    if root.name != run_id:
        root = root / run_id

    base = root / pdb_id.upper()
    if variant_token:
        base = base / str(variant_token).upper()

    receptor_root = base / "receptor"
    if ph_label:
        receptor_root = receptor_root / "ph_ensemble"
    suffix = f"_{ph_label}" if ph_label else ""
    candidate = receptor_root / f"{pdb_id.upper()}{suffix}.pdbqt"
...
def run_ligand_pipeline_subrun(ctx: ProteinDockingContext, subrun: SubrunSpec) -> None:
    ...
    for ph_label in ph_tags:
        ...
        if not rec_path.exists():
            fallback_rec = _run_scoped_receptor_fallback(
                cfg, paths.pdb_id, variant_token, ph_label
            )
            if fallback_rec is not None:
                logger.info(
                    "[router] receptor_file fallback_run_scoped original=%s resolved=%s",
                    str(rec_path),
                    str(fallback_rec),
                )
                rec_path = fallback_rec
        out_root = docked_dir(
            paths.pdb_id, variant=variant_token, ph_tag=ph_label, legacy=legacy_mode
        )
        ...
        try:
            ...
            stage1_original = [
                ...
                if norm(lig_path) not in control_norms_for_run
            ]
