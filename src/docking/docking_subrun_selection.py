...
from docking.docking_ligand_selection import select_ligands_for_next
...
def _apply_force_carry_and_doping(
    ...
    *,
    ...
    """
    rescue_pairs: List[Tuple[float, str]] = []
    rescue_ligands: List[str] = []
    if invalids and stage_index < len(stages_for_run) - 1:
        for lig, (sc, reason) in invalids.items():
            if (
                ...
                rescue_pairs.append((sc, lig))
        rescue_ligands = [
            lig
            for _, lig in sorted(rescue_pairs)[
                : int(cfg.get("RESCUE_SELF_RMSD_TOP_N", 10))
            ]
        ]
    ...
    if rescue_ligands:
        ...
        rescue_unique = [r for r in _filter_controls(rescue_ligands) if r not in sel_set]
