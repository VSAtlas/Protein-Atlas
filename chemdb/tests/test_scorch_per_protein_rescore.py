...
def test_scorch_runs_per_protein_after_docking(monkeypatch, tmp_path: Path) -> None:
    ...
    monkeypatch.setattr(main, "run_logs_dir", lambda _cfg: logs_dir)
    monkeypatch.setattr(
        main,
        "_prepare_run_logfile",
        lambda _run_id: str(tmp_path / "logs" / f"main_{_run_id}.log"),
    )
    monkeypatch.setattr(main, "_tee_stdio_to", _noop)
    monkeypatch.setattr(main, "bootstrap_root_logging", _noop)
    monkeypatch.setattr(main, "ensure_file_handler", _noop)
    monkeypatch.setattr(main, "init_run_manifest", _noop)
    monkeypatch.setattr(main, "update_manifest_for_config_hash", _noop)
    monkeypatch.setattr(main, "update_manifest_for_scheduled_proteins", _noop)
    monkeypatch.setattr(main, "update_manifest_for_run_config", _noop)
    monkeypatch.setattr(main, "update_manifest_for_protein_start", _noop)
    monkeypatch.setattr(main, "update_manifest_for_protein_success", _noop)
    monkeypatch.setattr(main, "update_manifest_for_protein_failure", _noop)
    monkeypatch.setattr(main, "apply_pocket_detection_events", _noop)
    monkeypatch.setattr(main, "finalize_run_manifest", _noop)
    monkeypatch.setattr(main, "apply_deepcoy_defaults", _noop)
    monkeypatch.setattr(main, "apply_deepcoy_cli_overrides", _noop)
    monkeypatch.setattr(main, "define_docking_stages", lambda _mode: ["stage1"])
    monkeypatch.setattr(main, "resolve_apo_holo_mode", lambda _cfg: ("apo", ["APO"]))
    monkeypatch.setattr(main, "_maybe_run_mmgbsa_for_pdb", _noop)
    monkeypatch.setattr(main, "_maybe_run_dud_eval", _noop)
    monkeypatch.setattr(main, "_maybe_run_master_schema_export", _noop)
    monkeypatch.setattr(main, "_maybe_run_report_generation", _noop)
    monkeypatch.setattr(main, "_log_rescore_verification", _noop)
