...
def test_run_openbabel_add_h_missing_tool_error_has_fix_commands(monkeypatch):
    monkeypatch.setattr(tool_runners.shutil, "which", lambda _name: None)

    with pytest.raises(RuntimeError) as excinfo:
        tool_runners.run_openbabel_add_h("/tmp/in.pdb", "/tmp/out.pdb")

    msg = str(excinfo.value)
    assert "Open Babel check failed" in msg
    assert "atlas --verify-tools" in msg
