...
def test_pick_reduce_exe_falls_back_to_path_lookup(tmp_path, monkeypatch):
    ...
    monkeypatch.delenv("REDUCE_BIN", raising=False)
    monkeypatch.setattr(protonation, "_REPO_ROOT", tmp_path)
    monkeypatch.setattr(protonation, "PHENIX_DIR", "")
    monkeypatch.setattr(protonation.shutil, "which", lambda name: "/usr/bin/reduce")
...
def test_assign_protonation_states_calls_openbabel_on_reduce_failure(
    ...
    monkeypatch.setattr(protonation, "run_subprocess_capture", fail_reduce)
