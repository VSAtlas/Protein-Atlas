...
def test_enumerate_ligands_falls_back_to_manifest_when_registry_paths_are_stale(tmp_path):
    lib_root = tmp_path / "prepped_ligands" / "fda_test_library_10"
    lib_root.mkdir(parents=True)
    for idx in range(1, 4):
        (lib_root / f"test_library_10_{idx}.pdbqt").write_text(
            "REMARK test ligand\n" + ("ATOM      1  C   LIG A   1       0.0   0.0   0.0\n" * 4),
            encoding="utf-8",
        )

    (lib_root / "microstates.json").write_text(
        json.dumps(
            {
                "library": "fda_test_library_10",
                "microstates": [
                    {
                        "microstate_id": "missing",
                        "pdbqt_path": "microstates/test_library_10_00001__missing.pdbqt",
                        "aliases": [
                            {
                                "ligand_stem": "test_library_10_00001",
                                "ph_label": "pH7_0",
                                "ph_value": 7.0,
                            }
                        ],
    ...
    (lib_root / "_manifest.json").write_text(
        json.dumps(
            {
                "entries": {
                    f"test_library_10_{idx}": f"test_library_10_{idx}.pdbqt"
                    for idx in range(1, 4)
                }
            }
        ),
    ...
    assert [p.name for p in pdbqts] == [
        "test_library_10_1.pdbqt",
        "test_library_10_2.pdbqt",
        "test_library_10_3.pdbqt",
    ]
