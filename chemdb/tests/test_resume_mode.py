...
from run_manifest import get_manifest_paths  # noqa: E402
...
def test_resume_mode(tmp_path: Path) -> None:
    ...
    resume_env.update({"PYTHONUNBUFFERED": "1", "CPU": "1", "MAX_PARALLEL_JOBS": "1"})
    initial_cmd = [
        str(MAIN_PY),
        "-test",
        "temp",
    ...
    if not cfg_snap_path.exists():
        ...
        _run_python(initial_cmd, env=resume_env, check=False)
        assert manifest_path.exists(), "Initial run did not create run_manifest.yaml"
        manifest = _load_manifest(manifest_path)
        paths = manifest.get("paths") or {}
        ...
        if not cfg_snap_path.exists():
            ...
            cfg_snap_path.write_text(yaml.safe_dump(load_inputs()))
    ...
    try:
        ...
        assert summary_completed is not None and int(summary_completed) >= 2
