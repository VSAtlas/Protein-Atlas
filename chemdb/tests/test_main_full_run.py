...
TEST_PDB_PATH = INPUT_FIXTURE_ROOT / f"{TEST_PDB_ID}.pdb"
...
TEST_LIBRARY_SUBDIR = "fda_test_library_10"
PREPPED_TEST_LIB = PREPPED_FIXTURE_ROOT / TEST_LIBRARY_SUBDIR

# Root under which we expect docked outputs for TEST to land:
# - no variant: outputs/docked/<RUN_ID>/TEST/...
# - variant:    outputs/docked/<RUN_ID>/TEST/APO/... or outputs/docked/<RUN_ID>/TEST/HOLO/...
# - variant+pH: outputs/docked/<RUN_ID>/TEST/APO/pH*/... or outputs/docked/<RUN_ID>/TEST/HOLO/pH*/...
DOCKED_ROOT = REPO_ROOT / "outputs" / "docked" / TEST_RUN_ID / TEST_PDB_ID
...
def _copy_prepped_fixture(tmp_path: Path) -> Path:
    ...
    shutil.copytree(PREPPED_TEST_LIB, prepped_root / TEST_LIBRARY_SUBDIR)
    return prepped_root
...
def _find_recent_summary(
    ...
    """
    Find the newest docking_score_summary.csv under outputs/docked/<RUN_ID>/TEST that was
    written or updated after start_time.

    If expected_variant is provided ('APO' or 'HOLO'), we also require the
    summary path to live under outputs/docked/<RUN_ID>/TEST/<expected_variant>/...
    """
...
def test_full_run_produces_summary_without_failures(
    label: str, env_overrides: dict[str, str], tmp_path: Path
) -> None:
    """
    ...
    under different APO_HOLO_MODE / PH_ENSEMBLE combinations and asserts:
      ...
      - at least one docking_score_summary.csv under outputs/docked/<RUN_ID>/TEST is
        updated by this run and contains at least one data row.
      - for APO/HOLO modes, the summary is located under outputs/docked/<RUN_ID>/TEST/APO/...
        or outputs/docked/<RUN_ID>/TEST/HOLO/... respectively.
    """
    ...
    prepped_root = _copy_prepped_fixture(tmp_path)
