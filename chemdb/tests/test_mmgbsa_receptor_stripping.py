...
def test_mmgbsa_receptor_stripping(tmp_path, monkeypatch):
    ...
    lines = [
        ...
        _pdb_line("ATOM", 2, "CA", "ALA", "A", 1, 1.2, 0.0, 0.0, "C"),
        _pdb_line("HETATM", 3, "ZN", "ZN", "A", 200, 0.5, 0.5, 0.5, "ZN"),
        _pdb_line("HETATM", 4, "O", "HOH", "A", 101, 1.0, 1.0, 1.0, "O"),
        _pdb_line("HETATM", 5, "O", "HOH", "A", 102, 2.0, 0.0, 0.0, "O"),
        _pdb_line("HETATM", 6, "O", "HOH", "A", 201, 10.0, 10.0, 10.0, "O"),
        _pdb_line("HETATM", 7, "O", "HOH", "A", 202, 12.0, 12.0, 12.0, "O"),
        "END\n",
    ]
    ...
    assert any(
        ...
        and line[22:26].strip() == "101"
        for line in out_lines
    )
    assert any(
        line.startswith(("ATOM", "HETATM"))
        and line[17:20].strip() == "HOH"
        and line[22:26].strip() == "102"
        for line in out_lines
    )
    assert not any(
        line.startswith(("ATOM", "HETATM"))
