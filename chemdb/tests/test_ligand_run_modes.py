...
def test_prepare_and_filter_ligands_dud_fda_modes(tmp_path):
    ...
    lig_root = Path(cfg["PREPPED_LIGANDS_DIR"])
...
def test_prepare_and_filter_ligands_keeps_symlinked_dud_library_entries(tmp_path):
    root = tmp_path / "dud_symlink_mode_test"
    root.mkdir()
    ...
    source_lig = _make_fake_lig(root / "source_ligands", "dud_test_00001")
    dud_root = Path(cfg["PREPPED_LIGANDS_DIR"]) / "dud_test"
    dud_root.mkdir(parents=True)
    linked_lig = dud_root / source_lig.name
    linked_lig.symlink_to(source_lig)

    paths = make_paths(cfg, base_id="TESTPDB", pdb_file="TESTPDB.pdb")
    logger = logging.getLogger("dud_symlink_mode_test")
    logger.setLevel(logging.INFO)

    ligands, heavy_atoms, pains = prepare_and_filter_ligands(cfg, paths, logger)

    assert ligands == [str(linked_lig)]
    assert heavy_atoms[str(linked_lig)] > 0
    assert pains[str(linked_lig)] is False
...
def test_prepare_and_filter_ligands_filters_ph_dirs_when_off(tmp_path):
    ...
    lib_root = Path(cfg["PREPPED_LIGANDS_DIR"]) / cfg["LIBRARY_SUBDIR_DEFAULT"]
...
def test_prepare_and_filter_ligands_chunk_keys_use_direct_manifest_lookup(tmp_path):
    root = tmp_path / "chunk_direct"
    root.mkdir()
    ...
    cfg = {
        ...
        "RUN_ID": "test_run",
        "_CHUNK_ID": "chunk-a",
        "_CHUNK_LIGAND_KEYS": ["dud_test_00002"],
    }

    for key in ("INPUT_DIR", "OUTPUT_DIR", "DOCKED_DIR", "PREPPED_LIGANDS_DIR"):
        Path(cfg[key]).mkdir(parents=True, exist_ok=True)

    lig_root = Path(cfg["PREPPED_LIGANDS_DIR"]) / "dud_test"
    _make_fake_lig(lig_root, "dud_test_00001")
    selected = _make_fake_lig(lig_root, "dud_test_00002")
    _make_fake_lig(lig_root, "dud_test_00003")

    paths = make_paths(cfg, base_id="TESTPDB", pdb_file="TESTPDB.pdb")
    logger = logging.getLogger("chunk_direct_test")
    logger.setLevel(logging.INFO)

    ligands, heavy_atoms, pains = prepare_and_filter_ligands(cfg, paths, logger)

    assert ligands == [str(selected)]
    assert cfg["_CHUNK_EFFECTIVE_LIGAND_KEYS"] == ["dud_test_00002"]
    assert heavy_atoms[str(selected)] > 0
    assert pains[str(selected)] is False
