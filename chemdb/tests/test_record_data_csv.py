...
from docking.ligand_metrics import write_scores_csv
...
def test_rerank_prefers_stage_matched_scorch_pick(tmp_path):
    logger = logging.getLogger("test.stage.matched.pick")
    ...
    scorch_rows = [
        {
            ...
            "ph": "pH7_2",
            "source": "vina",
            "Ligand_ID": "lig_a_stage3",
            "SCORCH_score": "0.9",
            "SCORCH_certainty": "1.0",
            "run_mode": "fda",
            "selected_stage": "stage1",
            "rescored_stage": "stage3",
            "selected_docking_score": "-8.0",
            "stage_match_flag": "0",
            "stage_fallback_reason": "preferred_missing:stage1",
        },
        {
            "pdb_id": "BOJG",
            "variant": "HOLO",
    ...
    assert len(rows) == 1
    row = rows[0]
    assert row["selected_stage"] == "stage1"
    assert row["rescored_stage"] == "stage1"
    assert row["stage_match_flag"] == "1"
    assert row["selected_docking_score"] == "-8"
