...
def mock_report_env(tmp_path):
    ...
    # 1. Master CSV
    # Columns: pdb_id, variant, ph_label, ligand_base, is_decoy, is_control, z_selected, z_selected_source, ef1, roc_auc
    ...
    rows = [
        # Target 1: T1|V1|P1
        {
            ...
            "consensus_sigma_decoy": "0.1",
            "consensus_n_decoys": "100",
            "blend_mu_decoy": "0.6",
        ...
        # Target 2: T2|V1|P1
        {
            ...
            "is_control": "0",
            "ef1": "5.0",
            "consensus_mu_decoy": "0.4",
            "consensus_sigma_decoy": "0.05",
            "consensus_n_decoys": "50",
            "blend_mu_decoy": "nan",
            "blend_sigma_decoy": "",
            "blend_n_decoys": "0",
        },
...
def test_report_generation(mock_report_env):
    ...
    assert l1_hits[0]["z_selected"] == 2.0
    assert l1_hits[1]["target"] == "T2|V1|P1"
    assert l1_hits[1]["z_selected"] == 3.0
...
def test_build_report_missing_master_rows_has_actionable_message(tmp_path: Path):
    repo_root = tmp_path / "repo"
    run_id = "MISSING_REPORT_INPUTS"
    (repo_root / "data" / run_id).mkdir(parents=True)
    (repo_root / "manifests" / run_id).mkdir(parents=True)

    with pytest.raises(FileNotFoundError) as excinfo:
        run_report.build_report(run_id, repo_root, top_n=5)

    msg = str(excinfo.value)
    assert "Report input check failed" in msg
    assert "master_rows CSV is missing" in msg
