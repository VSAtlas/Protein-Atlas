...
from docking.apo_holo_runtime import resolve_apo_holo_mode  # noqa: E402
from docking.docking_ligands import (  # noqa: E402
    _coerce_test_map,
    _is_under_ph_subdir,
)
from docking.library_mode import compute_allowed_library_roots  # noqa: E402
from config.runtime_config import load_inputs  # noqa: E402
...
def enumerate_ph_ligands_for_test_pdb(
    ...
    with patch.dict(os.environ, env, clear=False):
        ...
        ph_label = ph_tags[0] if ph_tags else "pH7_0"
        make_paths(cfg, base_id=chosen, pdb_file=f"{chosen}.pdb")
...
def test_main_fast_no_docking_excludes_ph_and_microstate_dirs(tmp_path: Path) -> None:
    ...
    with patch.dict(os.environ, env, clear=False):
        ...
        pdb_id = _pick_test_pdb(cfg_for_roots)
        env["ONLY_PDBS"] = pdb_id
    ...
    ph_roots = [lig_root]
    def is_micro(path: Path) -> bool:
        return any(part.lower() == "microstates" for part in path.parts)
    ...
    with patch.dict(os.environ, env, clear=False):
        ...
        for variant in variants:
            ...
            for prefix in expected_prefixes:
                ...
                if not planned and prefix == "":
                    # Base-library planning may emit run/fda suffixes depending on
                    # TEST_FDA overrides and mode normalization.
                    planned = list(variant_root.rglob("planned_ligands_*.txt"))
