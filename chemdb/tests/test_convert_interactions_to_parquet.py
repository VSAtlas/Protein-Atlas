...
from analysis import convert_interactions_to_parquet as converter
from analysis import heatmap_html
...
def test_convert_interactions_to_parquet_writes_partitions(tmp_path: Path) -> None:
    ...
    assert schema.field("t_selected").type == pa.float64()
...
def test_heatmap_reads_parquet_same_as_csv(tmp_path: Path) -> None:
    ...
    csv_scores = {key: value["t_selected"] for key, value in csv_agg.items()}
    parquet_scores = {key: value["t_selected"] for key, value in parquet_agg.items()}
