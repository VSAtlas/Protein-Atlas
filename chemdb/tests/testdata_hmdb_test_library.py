...
FIXTURE_ROOT = REPO_ROOT / "chemdb" / "tests" / "fixtures"
...
def ensure_hmdb_test_library(
    ...
    if not extracted_src.exists():
        fallback = FIXTURE_ROOT / "extracted_ligands" / "test_library_10"
        if fallback.exists():
            extracted_src = fallback
    if not prepped_src.exists():
        fallback = FIXTURE_ROOT / "prepped_ligands" / "test_library_10"
        if fallback.exists():
