...
def test_pocket_second_pass_no_sorting(tmp_path):
    ...
    for pocket_id in ("P1", "P2"):
        ...
        for idx in range(1, 11):
            rows.append(
                {
                    "ligand_id": f"S{idx}",
                    "score": -9.0 if pocket_id == "P1" else -6.2,
                    "label": "strong",
                }
            )
        for idx in range(1, 11):
        ...
        pockets.append(
            {
                "pocket_id": pocket_id,
                "metrics": {
                    "auc_mean": 0.90 if pocket_id == "P1" else 0.60,
                    "ef1_mean": 1.20 if pocket_id == "P1" else 0.80,
                    "auc_std": 0.0,
                },
    ...
    assert result["multi_pocket_detected"] is False
    assert result["selected_pockets"] == ["P1"]
    ...
    assert assignments_payload["selected_pockets"] == ["P1"]
    assert assignments_payload["multi_pocket_detected"] is False
    for entry in assignments_payload.get("assignments", []):
        assert entry["assigned_pocket_id"] == "P1"

    performance_payload = json.loads(performance_path.read_text(encoding="utf-8"))
    assert performance_payload["multi_pocket_detected"] is False
    assert performance_payload["selected_pockets"] == ["P1"]
    assert performance_payload["decision_reason"] == "clear_winner"
    assert performance_payload["pockets"]
