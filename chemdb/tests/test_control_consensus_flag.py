...
def test_select_control_engines_defaults_to_vina_only():
    ...
    engines = docking._select_control_engines(cfg, use_gnina=True, use_ledock=True, use_dock6=True)
    assert engines == ["vina"]
...
def test_select_control_engines_respects_disabled_capabilities():
    cfg = {"CONTROL_CONSENSUS": True}
    engines = docking._select_control_engines(cfg, use_gnina=False, use_ledock=True, use_dock6=False)
