...
RUN_ID = "screen_repro_6LU7_testfda_1768000719"
...
def _create_fake_logs(tmp_path: Path) -> None:
    ...
    failed_log.write_text(
        (
            ...
            "\n"
            "2026-01-09 12:00:00,000 - ERROR - root - Ligand preparation failed: non-zero exit status\n"
            "Traceback (most recent call last):\n"
            '  File "ligand_driver.py", line 1, in <module>\n'
            '    raise RuntimeError("ligand preparation crashed")\n'
            "RuntimeError: ligand preparation crashed\n"
        ),
        encoding="utf-8",
    )

    prep_dir = tmp_path / "docked" / RUN_ID / "6LU7" / "prep_work"
    prep_dir.mkdir(parents=True)
    (prep_dir / "ligand_prep.stderr.txt").write_text(
        (
            "Traceback (most recent call last):\n"
...
def test_summarize_run_errors_hermetic(tmp_path: Path) -> None:
    ...
    assert "docked/" in stdout
    assert "prep_work/" in stdout
    ...
    assert any(
        "/prep_work/" in _posix_path(example["path"])
