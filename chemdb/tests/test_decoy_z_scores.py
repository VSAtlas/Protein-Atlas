...
def test_decoy_z_scores_present_in_fast_test_mode() -> None:
    """
    ...
    non-zero decoy z-score column.
    """
    if not _have_test_inputs():
        pytest.skip("Skipping decoy z-score test: TEST inputs or test library missing.")

    assert MAIN_PY.exists(), f"main.py not found at {MAIN_PY}"

    env = os.environ.copy()
    # Ensure the code-path that mixes FDA + DUD is active.
    env["PREPPED_LIGANDS_DIR"] = str(PREPPED_FIXTURE_ROOT)
    ...
    with long_csv.open(newline="", encoding="utf-8") as f:
        ...
        if "z_vs_decoys" in reader.fieldnames:
            score_col = "z_vs_decoys"
        elif "t_vs_decoys" in reader.fieldnames:
            # Temporary compatibility fallback while remaining writers migrate.
            score_col = "t_vs_decoys"
        else:
            raise AssertionError(f"z_vs_decoys column missing in {long_csv}")
        z_values = []
        for row in reader:
            value_str = (row.get(score_col) or "").strip()
            if not value_str:
                continue
            try:
                z_values.append(float(value_str))
            except ValueError:
                continue

    assert z_values, f"No decoy z-score values recorded in {long_csv}"
    assert any(
        abs(val) > 1e-9 for val in z_values
    ), f"All decoy z-scores were zero in {long_csv}; expected decoy comparison to produce non-zero."
