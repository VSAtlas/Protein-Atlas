...
def mock_run_env(tmp_path):
    ...
    processed_root = repo_root / "outputs" / "processed_pdbs" / run_id
    processed = processed_root / pdb / "ligands_raw"
        ...
        "proteins": {
            pdb: {
                "stages": {
                    "pocket_detection": {
                        "details": {
                            "pocket_method": "configured_center",
                            "center_x": 10.0,
                            "center_y": 20.0,
                            "center_z": 30.0,
                        }
...
def test_master_export(mock_run_env):
    ...
    # Verify output
    out_csv = repo_root / "outputs" / "data" / run_id / "master_rows.csv"
    ...
    # Check Decoy
    decoy = next(
        r for r in rows if r["ligand_base"] == "decoys_LIG_FDA" and r["is_decoy"] == "1"
    )
    assert decoy["is_decoy"] == "1"
    assert decoy["z_stage2"] == "-1.5"

    # Verify Optional Artifacts
    top_targets_csv = repo_root / "outputs" / "data" / run_id / "ligand_top_targets.csv"
    ...
    qc_csv = repo_root / "outputs" / "data" / run_id / "target_qc_summary.csv"
