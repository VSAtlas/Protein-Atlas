...
COMMON_NAME_STEREOISOMER_RE = re.compile(
    r"^[A-Za-z][A-Za-z' -]+ \((?:\d+[RS](?:,\d+[RS])*) stereoisomer\)$"
)
...
IDENTIFIER_LIKE_PATTERNS: tuple[re.Pattern[str], ...] = (
    ...
    re.compile(r"^CID\s*\d+$", re.IGNORECASE),
    re.compile(r"^CHEBI:\d+$", re.IGNORECASE),
    re.compile(r"^DB\d+$", re.IGNORECASE),
    re.compile(r"^NCGC\d+(?:[-\s]?\d+)?$", re.IGNORECASE),
    re.compile(r"^MFCD\d+$", re.IGNORECASE),
    re.compile(r"^STARBLD\d+$", re.IGNORECASE),
)
NON_DRUG_NAME_PATTERNS: tuple[re.Pattern[str], ...] = (
    re.compile(r"^non[-\s]?live$", re.IGNORECASE),
)
...
NDC_LIKE_RE = re.compile(
    r"^(?:NDC[-\s:]*)?\d{4,6}-\d{2,4}(?:-\d{1,2})?$",
    re.IGNORECASE,
)
...
def is_ndc_like_name(name: str) -> bool:
    text = _collapse_spaces(name)
    if not text:
        return False
    return NDC_LIKE_RE.match(text) is not None
...
def display_name_quality_reasons(
    ...
    *,
    ...
    if any(pattern.match(text) is not None for pattern in NON_DRUG_NAME_PATTERNS):
        reasons.append("non_drug_alias")
    if is_cas_only_name(text):
        reasons.append("cas_only")
    if is_ndc_like_name(text):
        reasons.append("ndc_like")
    ...
    iupac_token_match = IUPAC_TOKEN_RE.search(text) and (
        PUBCHEM_PUNCT_RE.search(text) or "-" in text or " " in text or len(text) > 40
    )
    is_common_name_stereoisomer = COMMON_NAME_STEREOISOMER_RE.match(text) is not None
    if not is_common_name_stereoisomer and (
...
def _is_stereo_prefix_extension(candidate_key: str, current_key: str) -> bool:
    if not current_key or not candidate_key.endswith(current_key):
        return False
    prefix = candidate_key[: -len(current_key)]
    if not prefix:
        return False
    return re.fullmatch(r"(?:\d+)?[rsdl]+", prefix) is not None


def _existing_display_name_candidates(row: dict[str, Any]) -> list[str]:
    candidates: list[str] = []
    scalar_fields = (
    ...
    for field_name in scalar_fields:
        candidate = _collapse_spaces(row.get(field_name))
        if candidate:
            candidates.append(candidate)
    for field_name in list_fields:
        candidates.extend(_split_synonyms(_clean_text(row.get(field_name))))
    return _dedupe_preserve(candidates)


def _display_matches_trusted_name(row: dict[str, Any], display_key: str) -> bool:
    if not display_key:
        return False
    for field_name in (
        "pubchem_record_title",
        ...
        if _normalized_name_key(row.get(field_name)) == display_key:
            return True
    return False


def _quality_reasons_allowing_title(candidate: str, pubchem_iupac_name: str) -> list[str]:
...
def _trusted_candidate_is_relevant(
    candidate_key: str,
    current_key: str,
    row: dict[str, Any],
) -> bool:
    if not candidate_key or candidate_key == current_key:
        return False
    if len(current_key) <= 4 and len(candidate_key) > len(current_key):
        return True
    if (
        candidate_key.endswith(current_key)
        and not _is_stereo_prefix_extension(candidate_key, current_key)
        and len(candidate_key) - len(current_key) <= 5
    ):
        return True
    generic_key = _normalized_name_key(row.get("generic_name"))
    if len(generic_key) >= 5 and current_key.endswith(generic_key):
        return True
    return SequenceMatcher(None, current_key, candidate_key).ratio() >= 0.82


def _select_trusted_display_name_replacement(row: dict[str, Any]) -> str:
    current_display = _clean_text(row.get("display_name"))
    current_key = _normalized_name_key(current_display)
    current_is_placeholder = bool(
        FDA_PLACEHOLDER_RE.match(current_display) or UNK_PLACEHOLDER_RE.match(current_display)
    )
    pubchem_iupac_name = _clean_text(row.get("pubchem_iupac_name"))
    pubchem_iupac_key = _normalized_name_key(pubchem_iupac_name)
    for field_name in (
        "drugcentral_generic_name",
        "pubchem_record_title",
        "rxnorm_generic_name",
        ...
        candidate = _collapse_spaces(row.get(field_name))
        candidate_key = _normalized_name_key(candidate)
        if not current_is_placeholder and not _trusted_candidate_is_relevant(
            candidate_key,
            current_key,
            row,
        ):
            continue
        if candidate_key == pubchem_iupac_key:
            continue
        if _quality_reasons_allowing_title(candidate, pubchem_iupac_name):
            continue
        return candidate
    return ""


def _candidate_is_existing_display_replacement(
    candidate: str,
    current_display: str,
    ...
    if any(
        ...
        return False
    if _is_stereo_prefix_extension(candidate_key, current_key):
        return False
    if len(current_key) <= 4 and len(candidate_key) > len(current_key):
        return True
    return (
        len(current_key) >= 5
        and candidate_key.endswith(current_key)
        and len(candidate_key) - len(current_key) <= 3
    )


def row_display_name_quality_reasons(
    row: dict[str, Any],
    *,
    strict: bool = False,
) -> list[str]:
    ...
    if strict and display_name:
        current_key = _normalized_name_key(display_name)
        if _display_matches_trusted_name(row, current_key):
            return _dedupe_preserve(
                reason for reason in reasons if reason != "iupac_like"
            )
        for candidate in _existing_display_name_candidates(row):
            if not _candidate_is_existing_display_replacement(
                candidate,
                display_name,
                pubchem_iupac_name,
            ):
            ...
            elif candidate_key.endswith(current_key):
                reasons.append("truncated_display_name")
            break
    return _dedupe_preserve(reasons)


def _select_existing_display_name_replacement(row: dict[str, Any]) -> str:
    current_display = _clean_text(row.get("display_name"))
    pubchem_iupac_name = _clean_text(row.get("pubchem_iupac_name"))
    trusted_replacement = _select_trusted_display_name_replacement(row)
    if trusted_replacement:
        return trusted_replacement
    for candidate in _existing_display_name_candidates(row):
        if _candidate_is_existing_display_replacement(
            candidate,
            current_display,
            pubchem_iupac_name,
        ):
...
def _process_row(
    ...
    display_bad = bool(
        row_display_name_quality_reasons(row, strict=strict_quality_mode)
    )
    ...
    if evaluate and display_bad:
        resolved_name = ""
        existing_replacement = _select_existing_display_name_replacement(row)
        replacement_row = {**row, "display_name": existing_replacement}
        if existing_replacement and not row_display_name_quality_reasons(
            replacement_row,
            strict=strict_quality_mode,
        ):
            if existing_replacement != current_display:
                row["display_name"] = existing_replacement
                row_updated = True
            resolved_name = existing_replacement
            LOGGER.info(
                "[existing-name] row=%s name=%s",
                row_index,
        ...
        if (
            not resolved_name
            and rxnorm_name
            and is_good_display_name(rxnorm_name, pubchem_iupac)
        ):
        ...
        if not resolved_name:
            current_is_placeholder = bool(
                FDA_PLACEHOLDER_RE.match(current_display)
                or UNK_PLACEHOLDER_RE.match(current_display)
            )
            if not current_display or current_is_placeholder:
    ...
    final_quality_reasons = row_display_name_quality_reasons(
        row,
        strict=strict_quality_mode,
    )
